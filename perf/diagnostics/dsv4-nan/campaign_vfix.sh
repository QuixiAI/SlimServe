#!/bin/bash
# Multi-boot arm: TP4 with FULL_DECODE_ONLY capture-32 graphs (the TP2 profile's
...
for BOOT in 1 2 3 4 5 6 7 8; do
  ...
  sudo systemd-run --unit=ss-$NAME --uid=ubuntu --gid=ubuntu \
    ...
    -E VLLM_NAN_WATCH=1 -E VLLM_NAN_WATCH_LAYERS=1 -E VLLM_DSV4_TOPK_VALIDATE=1 -E VLLM_DSV4_ALIGNED_Q8=1 -E VLLM_DSV4_MHC_SCHEDULE=async \
    ...
    --model "$MODEL" --host 127.0.0.1 --port 1806$BOOT --tensor-parallel-size 4 --kv-cache-dtype fp8 --block-size 256 \
  ...
  for i in $(seq 1 100); do
    c=$(curl -s -o /dev/null -w '%{http_code}' -m 5 http://127.0.0.1:1806$BOOT/health 2>/dev/null)
    [ "$c" = "200" ] && break
    sleep 10
  done
  for i in 1 2; do
    /home/ubuntu/SlimServe/.venv/bin/python /home/ubuntu/SlimServe/benchmarks/benchmark_dsv4_exact.py \
      ...
      --url http://127.0.0.1:1806$BOOT/v1/completions --concurrency 11 --input-tokens 1000 --output-tokens 2000 \
...
print(t)")
  echo "VFIX BOOT $BOOT: degenerate_reqs=$DEG nan_lines=${NAN:-0} validate_lines=${VAL:-0}"
  sudo systemctl stop ss-$NAME 2>/dev/null
  sleep 15
done
echo "VFIX CAMPAIGN DONE"
