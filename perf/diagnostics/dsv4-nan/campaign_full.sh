...
for BOOT in 1 2 3 4 5 6; do
  ...
  sudo systemd-run --unit=ss-$NAME --uid=ubuntu --gid=ubuntu \
    ...
    --model "$MODEL" --host 127.0.0.1 --port 1803$BOOT --tensor-parallel-size 4 --kv-cache-dtype fp8 --block-size 256 \
  ...
  for i in $(seq 1 100); do
    c=$(curl -s -o /dev/null -w '%{http_code}' -m 5 http://127.0.0.1:1803$BOOT/health 2>/dev/null)
    [ "$c" = "200" ] && break
    sleep 10
  done
  for i in 1 2; do
    /home/ubuntu/SlimServe/.venv/bin/python /home/ubuntu/SlimServe/benchmarks/benchmark_dsv4_exact.py \
      ...
      --url http://127.0.0.1:1803$BOOT/v1/completions --concurrency 11 --input-tokens 1000 --output-tokens 2000 \
...
print(t)")
  echo "BOOT $BOOT: degenerate_reqs=$DEG nan_lines=${NAN:-0} validate_lines=${VAL:-0}"
