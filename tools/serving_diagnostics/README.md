...
also uses `httpx`, `anyio`, `starlette`, and `uvicorn` from the serving environment.

## Proxy on port 8000, SlimServe on port 8001

Start a registered profile for your hardware on `127.0.0.1:8001`, then run:
...
the capture limit does not stop forwarding. Request bodies are buffered in full. This directory contains no service
