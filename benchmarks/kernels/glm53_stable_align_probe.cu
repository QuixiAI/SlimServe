...
void run_into(Tensor ids, Tensor sorted, Tensor experts, Tensor padded,
              Tensor offsets, int64_t block, bool parallel_count) {
    ...
    auto counter = parallel_count ? tms::glm_stable_align::count_prefix<1024>
                                  : tms::glm_stable_align::count_prefix<256>;
...
std::vector<Tensor> run(Tensor ids, int64_t block, bool parallel_count) {
    ...
    run_into(ids, sorted, experts, padded, offsets, block, parallel_count);
    return {sorted, experts, padded};
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("run", &run, pybind11::arg("ids"), pybind11::arg("block"),
          pybind11::arg("parallel_count") = false);
