...
void run_into(Tensor ids, Tensor sorted, Tensor experts, Tensor padded,
    ...
    tms::glm_stable_align::count_prefix<<<2, 256, 0, stream>>>(
...
std::vector<Tensor> run(Tensor ids, int64_t block) {
    ...
    run_into(ids, sorted, experts, padded, offsets, block);
    return {sorted, experts, padded};
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("run", &run);
    m.def("run_into", &run_into);
}
