            ...
            for mode in ("control", "legacy"):
                ...
                with torch.cuda.device(rank):
                    ...
                    template = load_source(
                        copied, kernel_name, f"rms_smoke_{rank}_{mode}"
                    )
                    ...
                    expected = compile_config(entry["configs"][int(mode == "control")])
                    ...
                    future.reload_kernel_from_src = lambda template=template: template
                    ...
                    for rows in (16, 640):
                        x, changed = [
                            make_inputs(rows, seed, 1.0) for seed in (530901, 531001)
                        ]
