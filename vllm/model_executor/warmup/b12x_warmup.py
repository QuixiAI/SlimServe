...
"""Compile the b12x JIT kernels a loaded model will launch, before any CUDA
graph capture can hit a first-use compile."""
...
from vllm.utils.b12x import B12xWarmupUnit, b12x_warmup_token_counts, has_b12x
...
def b12x_warmup(worker: "Worker", cudagraph_capture_sizes: list[int]) -> None:
    ...
    token_counts = b12x_warmup_token_counts(max_tokens=max_tokens, sizes=serving_sizes)
    ...
    for name, count in _compile_warmup_units(units).items():
        logger.info_once("Warmed up %d b12x %s kernel signature(s).", count, name)
