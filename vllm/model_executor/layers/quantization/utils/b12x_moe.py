...
"""ModelOpt NVFP4 MoE tensors in the layout b12x's fused MoE consumes."""
...
# b12x tiles the gated FC1 rows in 64s.
_GATED_ROW_MULTIPLE = 64
...
def _pad_gated_weights(
    ...
    half_pad_size = round_up(half_size, _GATED_ROW_MULTIPLE) - half_size
    if half_pad_size == 0:
...
def prepare_nvfp4_moe_layer_for_b12x(
    ...
    """Pad the intermediate rows to b12x's tile, swizzle the block scales and
    flatten the per-expert activation scales. The packed FP4 payload keeps
    vLLM's gate-first row order (b12x's ``w31`` layout) and the weight global
    scales are passed through raw."""
