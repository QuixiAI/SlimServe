...
def _resolve_gguf_weight_loader(
    ...
    # Layers with no v2 loader -- notably ReplicatedLinear, which the MLA models
    # use for q_a_proj, kv_a_proj_with_mqa and the DSA indexer's wq_b -- would
    # otherwise fall back to the layer's own weight_loader. That one asserts
    # `param.size() == loaded_weight.size()`, which always fails against a GGUF
    # parameter that is still an empty placeholder awaiting materialization.
    # Replicated layers do no sharding, so storing the packed tensor is the
    # entire contract.
    del fallback_weight_loader
    return _store_gguf_loaded_weight
    ...
    if not isinstance(param, UninitializedParameter):
        if param.data.numel() == 0:
            param.data = torch.empty(
                num_elements, dtype=torch.uint8, device=param.device
            )
...
class _GGUFParamLoadMixin:
    ...
    def load_qkv_weight(self, loaded_weight: torch.Tensor, **kwargs):
        # Mirrors BasevLLMParameter.load_qkv_weight: everything that is not "q"
        # divides by the replication factor, which is how a fully replicated
        ...
        if (
            ...
            loaded_weight = loaded_weight.narrow(
                0, shard_id_int * shard_size, shard_size
            )
        self._store(loaded_weight, shard_id=shard_id)
