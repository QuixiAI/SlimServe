...
def _use_a16(backend: NvFp4MoeBackend, checkpoint_uses_a16: bool) -> bool:
    # b12x can run bf16 activations over the W4A4 checkpoint's weights.
...
def select_nvfp4_moe_backend(
    ...
    for backend in AVAILABLE_BACKENDS:
        # The bf16-activation override the explicit-backend branch applies
        # (VLLM_B12X_MOE_FP4_FORCE_A16), so both branches select against the
        # activation the layer conversion then builds.
        backend_activation_key = None if _use_a16(backend, False) else activation_key
        for k_cls in backend_to_kernel_cls(backend):
            supported, reason = k_cls.is_supported_config(
                k_cls,
                config,
