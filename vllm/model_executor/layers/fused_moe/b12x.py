...
"""NVFP4 routed experts on b12x's fused tensor-parallel MoE (SM12x).

b12x owns the whole routed-expert pipeline for one launch: activation
quantization, expert-row gathering, both grouped GEMMs, the activation and the
weighted reduce back to token order. This class hands it the ModelOpt NVFP4
tensors once (``process_weights_after_loading``), keeps one execution plan per
token capacity, and passes the modular kernel's ``workspace2`` as the plan's
scratch. Plans are created and their kernels compiled before CUDA graph
capture (``warmup/b12x_warmup.py``); ``bind`` and ``run`` never allocate.
"""
...
# (weight quant dtype, activation quant dtype) -> b12x quant mode.
_B12X_MOE_MODES: dict[tuple[str, str | None], str] = {
    # W4A4: FP4 activations with E4M3 K/16 scales, the checkpoint's recipe.
    ("nvfp4", "nvfp4"): "nvfp4",
    # MXFP8 activations over the same NVFP4 weights.
    ("nvfp4", "mxfp8"): "w4a8_nvfp4",
    # bf16 activations, FP4 weights dequantized inline.
    ("nvfp4", None): "w4a16",
}
_SOURCE_FORMAT = "modelopt_nvfp4"
# b12x's name for vLLM's gate-first ``[w1; w3]`` FC1 row order.
_W13_LAYOUT = "w31"
...
def _as_int32_ids(topk_ids: torch.Tensor) -> torch.Tensor:
    if topk_ids.dtype == torch.int32 and topk_ids.is_contiguous():
        return topk_ids
    if _is_current_stream_capturing():
        raise RuntimeError("b12x MoE topk_ids conversion would allocate during capture")
    return topk_ids.to(dtype=torch.int32).contiguous()


def _as_f32_weights(topk_weights: torch.Tensor) -> torch.Tensor:
    if topk_weights.dtype == torch.float32 and topk_weights.is_contiguous():
        return topk_weights
    if _is_current_stream_capturing():
        raise RuntimeError(
            "b12x MoE topk_weights conversion would allocate during capture"
        )
    return topk_weights.to(dtype=torch.float32).contiguous()


def _expert_scale_vector(scale: torch.Tensor) -> torch.Tensor:
    if scale.ndim == 2:
...
def _empty_parameter(layer: torch.nn.Module, name: str) -> torch.Tensor | None:
...
class B12xExperts(mk.FusedMoEExpertsModular):
    """ModelOpt NVFP4 experts on the b12x planned MoE API."""
    ...
    def __init__(
        ...
        scheme = (quant_config.weight_quant_dtype, quant_config.quant_dtype)
        try:
            self._quant_mode = _B12X_MOE_MODES[scheme]
    ...
    def _weight_global_scale(
        ...
        scale = _expert_scale_vector(scale)
    ...
    def _prepare_experts(
        ...
        *,
        ...
        if _is_current_stream_capturing():
            raise RuntimeError("b12x MoE weights must be prepared before capture")
        ...
        if self._quant_mode == "w4a16":
            # bf16 activations: no activation-scale arithmetic in the kernel.
            a1_gscale = self._unit_scale(w1.device, num_experts)
            a2_gscale = self._unit_scale(w2.device, num_experts)
        else:
            if self.a1_gscale is None or self.a2_gscale is None:
                raise ValueError("b12x NVFP4 MoE requires activation global scales")
            a1_gscale = _expert_scale_vector(self.a1_gscale).to(w1.device)
            a2_gscale = _expert_scale_vector(self.a2_gscale).to(w2.device)
    ...
    def _release_source_parameters(self, layer: torch.nn.Module) -> None:
        if self._source_parameters_released:
            return
        w1_scale = _empty_parameter(layer, "w13_weight_scale")
        w2_scale = _empty_parameter(layer, "w2_weight_scale")
        if w1_scale is not None:
            self.quant_config._w1.scale = w1_scale
        if w2_scale is not None:
            self.quant_config._w2.scale = w2_scale
        _empty_parameter(layer, "w13_weight")
        _empty_parameter(layer, "w2_weight")
    ...
    def _supports_current_device() -> bool:
        if not (
            ...
            return False
        fused_moe = get_b12x_fused_moe()
        return fused_moe is not None and fused_moe.is_supported()
    ...
    def moe_problem_size(
        ...
        # The source parameters were released: read the prepared geometry.
    ...
    def _plan(
        ...
        *,
        ...
        if plan is not None:
            return plan
        if _is_current_stream_capturing():
            raise RuntimeError("b12x MoE plans must be created before capture")
    ...
    def warmup_launches(
        ...
        *,
        ...
        """Create the plan for every token capacity and launch each distinct
        kernel signature once, so nothing compiles under graph capture."""
        fused_moe = _require_b12x_fused_moe()
        ...
        for tokens in sorted(set(token_counts)):
            execution_plan = fused_moe.plan_execution(
                num_tokens=max(tokens, 1),
                num_topk=topk,
                device=device,
                weight_plan=prepared.plan,
                quant_mode=self._quant_mode,
        ...
        # Warmup launches may outlive this call on the device: keep every
        # tensor alive until the synchronize so the allocator cannot reuse an
        # address a kernel still reads.
        ...
        for tokens in launch_tokens.values():
            ...
            for route_ids_dtype in (torch.int32, torch.int64):
                scratch = torch.empty(
                    (_b12x_scratch_nbytes(plan),), dtype=torch.uint8, device=device
                )
                hidden_states = torch.zeros(
                    (tokens, int(prepared.hidden_size)), dtype=dtype, device=device
                )
                ...
                self._run(
                    ...
                    topk_weights=topk_weights,
                    topk_ids=_as_int32_ids(topk_ids),
                    output=output,
                )
        if launch_resources:
    ...
    def apply(
        ...
        if bool(apply_router_weight_on_input) != self._apply_router_weight_on_input:
            raise ValueError(
                "apply_router_weight_on_input does not match the prepared b12x plan"
            )
        if workspace2 is None:
            raise RuntimeError("b12x MoE requires the workspace2 scratch")
        ...
        needed = _b12x_scratch_nbytes(plan)
        if scratch.numel() < needed:
            raise ValueError(
                f"b12x MoE workspace2 is too small: have={scratch.numel()} "
                f"bytes, need={needed} bytes"
            )
        self._run(
            plan=plan,
            scratch=scratch,
            hidden_states=hidden_states,
            topk_weights=_as_f32_weights(topk_weights),
            topk_ids=_as_int32_ids(topk_ids),
            output=output,
        )
