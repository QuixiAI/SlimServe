...
"""Metal NVMe KV tier: whole-slab transfers on unified memory.

This connector uses per-group positional histories, including finer sliding
windows. CUDA/ROCm host and disk residency use host_tier_connector instead.
The separate module keeps their different storage and completion contracts
explicit; Metal profiles select this module via kv_connector_module_path.
...
Config (kv_transfer_config.kv_connector_extra_config):
    nvme_tier_gb_per_rank: NVMe arena size per rank in GiB (required).
"""
...
if TYPE_CHECKING:
    ...
    from vllm.v1.worker.gpu.metal_kv_tier_nvme import KVTierNVMe
...
class HostTierConnector(KVConnectorBase_V1, SupportsHMA):
    def __init__(
        ...
        if host_gb > 0 or nvme_gb <= 0:
            raise ValueError(
                "Metal requires nvme_tier_gb_per_rank > 0 and no host RAM tier"
            )
        tier_gb = nvme_gb
        ...
        if role == KVConnectorRole.SCHEDULER:
            ...
            self._dma: KVTierNVMe | None = None
    ...
    def start_load_kv(self, forward_context: ForwardContext, **kwargs) -> None:
        ...
        if not isinstance(meta, HostTierMeta) or self._dma is None:
            return
        from vllm.v1.worker.gpu.metal_kv_tier_nvme import TierOpBatch
