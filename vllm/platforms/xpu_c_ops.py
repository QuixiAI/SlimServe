...
def _swigluoai_and_mul(
    result: torch.Tensor, input: torch.Tensor, alpha: float = 1.702, limit: float = 7.0
) -> None:
...
def _gelu_tanh_and_mul(result: torch.Tensor, input: torch.Tensor) -> None:
    d = input.shape[-1] // 2
    result.copy_(
        torch.nn.functional.gelu(input[..., :d], approximate="tanh") * input[..., d:]
    )
...
def _gelu_fast(result: torch.Tensor, input: torch.Tensor) -> None:
    x = input.float()
    result.copy_(
        (0.5 * x * (1.0 + torch.tanh(x * 0.7978845608 * (1.0 + 0.044715 * x * x)))).to(
            input.dtype
        )
    )
...
_SCHEMAS = {
    (
        "rms_norm(Tensor! result, Tensor input, Tensor? weight, "
        "float epsilon) -> ()"
    ): _rms_norm,
    (
        "fused_add_rms_norm(Tensor! input, Tensor! residual, "
    ...
    (
        "silu_and_mul_with_clamp(Tensor! result, Tensor input, "
        "float limit, float alpha=1.0, float beta=0.0) -> ()"
    ...
    (
        "situ_and_mul(Tensor! out, Tensor input, float beta=1.0, "
        "float linear_beta=-1.0) -> ()"
    ): _situ_and_mul,
...
def register_xpu_c_ops() -> None:
    ...
    for schema, fn in _SCHEMAS.items():
        ...
        registered.append(name)
    logger.info_once(
        "Registered %d torch.ops._C XPU ops: %s", len(registered), ", ".join(registered)
    )
