...
def _tq_decode_stage1(
    ...
    SLIDING_WINDOW: tl.constexpr = 0,  # 0 = full attention
):
    ...
    # Causal sliding window: the query (at position seq_len - 1) attends to
    # the last SLIDING_WINDOW positions, i.e. [seq_len - SLIDING_WINDOW, seq_len).
    kv_start = 0
    if SLIDING_WINDOW > 0:
        kv_start = tl.maximum(seq_len - SLIDING_WINDOW, 0)

    # KV split range over the visible span
    split_len = tl.cdiv(seq_len - kv_start, NUM_KV_SPLITS)
