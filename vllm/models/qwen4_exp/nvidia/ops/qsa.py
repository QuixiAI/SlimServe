...
def _qsa_sparse_paged_gqa_splitk_kernel(
    ...
    for tile in range(split_tile_start, split_tile_end):
        ...
        if KV_TQ:
            # TurboQuant k8v4 slab: per (token, head) slot =
            # [K: HEAD_DIM bytes fp8 | V: HEAD_DIM/2 bytes nibbles |
            #  scale f16 | zero f16]. Keys are e4b15 on Ampere (Triton's
            # software fp8), e4nv on SM89+, matching _tq_fused_store_fp8.
            slot_base = (
                safe_page * stride_k_block
                + page_offset.to(tl.int64) * stride_k_token
                + kv_head * stride_k_head
            )
            k_raw = tl.load(
                k_cache_ptr + slot_base[None, :] + dim_offsets[:, None],
                mask=valid[None, :],
                other=0,
            )
            if TQ_E4B15:
                keys = (
                    k_raw.to(tl.float8e4b15, bitcast=True)
                    .to(tl.float32)
                    .to(tl.bfloat16)
                )
            else:
                keys = (
                    k_raw.to(tl.float8e4nv, bitcast=True)
                    .to(tl.float32)
                    .to(tl.bfloat16)
                )
            v_base = slot_base + TQ_KPS
            v_raw = tl.load(
                v_cache_ptr + v_base[:, None] + (dim_offsets[None, :] // 2),
                mask=valid[:, None],
                other=0,
            ).to(tl.int32)
...
def qsa_sparse_paged_attention(
    ...
    by ``triton_turboquant_store``.
    """

    kv_tq = tq_slot_size is not None
    if not q.is_cuda or not HAS_TRITON:
    ...
    head_dim = q.shape[2]
    if kv_tq:
        if k_cache.dtype != torch.uint8 or k_cache.shape[3] != tq_slot_size:
            raise ValueError("QSA TQ cache must be a uint8 [b, bs, h, slot] slab")
        if tq_slot_size < head_dim + head_dim // 2 + 4:
            raise ValueError("QSA TQ slot is too small for k8v4 at this head_dim")
    ...
    _qsa_sparse_paged_gqa_splitk_kernel[partial_grid](
        ...
        TQ_E4B15=(
            torch.cuda.get_device_capability(q.device)[:2] < (8, 9)
            if kv_tq
            else True
        ),
