...
def _ple_host_gather_kernel(
    ...
    """Gather ``ROW_BYTES``-byte rows from a pinned host table over UVA."""
    pid = tl.program_id(0)
    if pid >= num_rows:
        return
    row = tl.load(ids_ptr + pid).to(tl.int64)
    base = table_ptr_int.to(tl.pointer_type(tl.uint8))
    offs = tl.arange(0, BLOCK)
    mask = offs < ROW_BYTES
    vals = tl.load(base + row * ROW_BYTES + offs, mask=mask)
    tl.store(out_ptr + pid.to(tl.int64) * ROW_BYTES + offs, vals, mask=mask)


def qwen4_exp_ple_host_gather(
    ids: torch.Tensor,
    table: torch.Tensor,
) -> torch.Tensor:
    """Gather pinned-host table rows over UVA.

    A functional custom op so torch.compile treats it opaquely, and the table
    is a tensor argument so its host address is read at call time -- baking
    ``data_ptr()`` into the graph poisons vLLM's cross-process compile cache
    with a stale pointer.
    """
    num_rows = ids.numel()
    row_bytes = table.shape[1] * table.element_size()
    out = torch.empty(
        num_rows, row_bytes, dtype=torch.uint8, device=ids.device
    )
    if num_rows:
        block = triton.next_power_of_2(row_bytes)
        _ple_host_gather_kernel[(num_rows,)](
            table.data_ptr(),
            ids.reshape(-1),
            out,
            num_rows,
...
def qwen4_exp_ple_host_gather_fake(
    ids: torch.Tensor,
    table: torch.Tensor,
) -> torch.Tensor:
    row_bytes = table.shape[1] * table.element_size()
    return torch.empty(
        ids.numel(), row_bytes, dtype=torch.uint8, device=ids.device
    )


direct_register_custom_op(
    op_name="qwen4_exp_ple_host_gather",
    op_func=qwen4_exp_ple_host_gather,
    fake_impl=qwen4_exp_ple_host_gather_fake,
)


class _FullVocabShardIndices:
    """Shard-indices shim: the host table covers the whole vocabulary."""
...
class Qwen4ExpHostNGramEmbedding(nn.Module):
    ...
    Every rank keeps the full FP8 table pinned in host RAM and gathers the
    16 rows each token needs (2560 bytes) directly from a GPU kernel through
    zero-copy host reads. Compared with the TP-sharded VRAM table this frees
    the table's VRAM and removes the per-step embedding all-reduce, at the
    price of host RAM per rank; the gather is graph-capturable, so FULL
    cudagraph decode keeps working.
    """
    ...
    def __init__(
        ...
        if params_dtype == torch.float8_e4m3fn:
            self.weight_scale = nn.Parameter(
                torch.ones(1, dtype=torch.bfloat16), requires_grad=False
            )

    def forward(self, ids: torch.Tensor) -> torch.Tensor:
        rows = torch.ops.vllm.qwen4_exp_ple_host_gather(
            ids.reshape(-1), self.weight
        )
        return rows.view(self.weight_dtype).view(*ids.shape, self.embedding_dim)
    ...
    def __init__(
        ...
        if envs.VLLM_QWEN4_EXP_PLE_HOST:
            ...
            self.ngram_embedding = VocabParallelEmbedding(
                ...
                quant_method=embedding_quant_method,
            )
        self.register_buffer(
            "positions_buffer",
...
class Qwen4ExpPLELayer(nn.Module, MambaBase):
    ...
    def _short_conv_dilated_prefill_batched(
        ...
        # The packed pipeline materializes ~5 dense [rows, max_len, hidden]
        # transients. A wave of concurrent long prefills can push that past
        # the profiled activation reserve (observed: 31 x ~2K-token chat
        # turns OOMing at gpu_memory_utilization 0.95), so bound the
        # transient by processing requests in slices. The common case
        # (product under the budget) remains a single full-batch pass.
        slice_budget_elems = 16 << 20
        rows_per_slice = max(1, slice_budget_elems // max(1, max_len * hidden_size))
        conv_w = conv_weights.unsqueeze(1).contiguous()
        token_positions = torch.arange(max_len, device=x_p.device, dtype=torch.int64)

        for row_start in range(0, num_prefills, rows_per_slice):
            row_end = min(row_start + rows_per_slice, num_prefills)
            rows = row_end - row_start
            if rows == num_prefills:
                tok_idx = None
                req_rel = req_indices
                col_rel = col_indices
                x_slice = x_p
            else:
                sel = (req_indices >= row_start) & (req_indices < row_end)
                tok_idx = sel.nonzero(as_tuple=True)[0]
                req_rel = req_indices[tok_idx] - row_start
                col_rel = col_indices[tok_idx]
                x_slice = x_p[tok_idx]

            packed_tokens = x_p.new_zeros((rows, max_len, hidden_size))
            packed_tokens[req_rel, col_rel] = x_slice
            packed_tokens = packed_tokens.transpose(1, 2).contiguous()

            state_idx_s = state_indices[row_start:row_end]
            valid_state_s = valid_state[row_start:row_end]
            has_initial_s = has_initial[row_start:row_end]
            lengths_s = lengths[row_start:row_end]

            if self.conv_state_len > 0:
                if conv_state.shape[0] == 0:
                    state = conv_state.new_zeros(
                        (rows, hidden_size, self.conv_state_len),
                        dtype=x_p.dtype,
                    )
                else:
                    state = conv_state.index_select(0, state_idx_s)[
                        ..., : self.conv_state_len
                    ].to(x_p.dtype)
                use_initial_mask = (valid_state_s & has_initial_s).view(rows, 1, 1)
                initial_state = torch.where(
                    use_initial_mask,
                    state,
                    torch.zeros_like(state),
            ...
            valid_tokens = token_positions.view(1, max_len) < lengths_s.view(rows, 1)
            valid_output_mask = valid_tokens & valid_state_s.to(
                device=x_p.device
            ).view(rows, 1)
            conv_output.masked_fill_(~valid_output_mask.unsqueeze(-1), 0)
            if tok_idx is None:
                output.copy_(conv_output[req_rel, col_rel])
            else:
                output[tok_idx] = conv_output[req_rel, col_rel]

            if self.conv_state_len > 0 and conv_state.shape[0] > 0:
                state_starts = lengths_s.to(
                ...
                # Write back without a host synchronization. Valid, non-empty
                # rows receive their new state; padding and zero-length rows
                # keep the current cache value.
                existing_state = conv_state.index_select(0, state_idx_s)
                existing_base_state = existing_state[..., : self.conv_state_len]
                update_mask = valid_state_s & (
                    lengths_s.to(device=conv_state.device) > 0
                )
                safe_next_state = torch.where(
                    update_mask.view(rows, 1, 1),
                    next_state.to(conv_state.dtype),
                    existing_base_state,
                )
                existing_state[..., : self.conv_state_len] = safe_next_state
                conv_state.index_copy_(0, state_idx_s, existing_state)
        return output
