...
try:
    ...
    _HAS_SKINNY_GEMM = True
except ImportError:
    # This fork has no cute_dsl skinny GEMM; the whole plan table is gated on
    # SM103, so other platforms fall through to the standard linear path.
    _HAS_SKINNY_GEMM = False

    class SkinnyGemmConfig:  # type: ignore[no-redef]
    ...
    class _NoSkinnyGemm:
        ...
        @staticmethod
        def request_warmup_configs(*args, **kwargs) -> None:
            del args, kwargs

    shape_dynamic_skinny_gemm = _NoSkinnyGemm()  # type: ignore[assignment]
