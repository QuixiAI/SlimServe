...
@triton.jit
def _gather_bf16_k_kernel(
    out_ptr,
    out_stride0,
    out_stride1,
    k_cache_ptr,  # bf16 [num_blocks, block_size, 512] (dim0 may be strided)
    seq_lens_ptr,
    block_table_ptr,
    offset,
    gather_lens_ptr,
    ...
    block_stride: tl.constexpr,  # elements between blocks (packed-slab aware)
    token_stride: tl.constexpr,  # elements between token rows
    output_dim: tl.constexpr,  # 512
):
    """Paged gather for plain bf16 KV rows: same walk as the dequant
    gather, but the page already holds the workspace dtype so each token is
    one vectorized row copy (no scales, no decode)."""
    batch_idx = tl.program_id(0)
    worker_id = tl.program_id(1)
    num_workers = tl.num_programs(1)
    ...
    for i in range(worker_id, gather_len, num_workers):
        ...
        dst_ptr = out_ptr + batch_idx * out_stride0 + (offset + i) * out_stride1
        offs = tl.arange(0, output_dim)
        tl.store(dst_ptr + offs, tl.load(src_ptr + offs))


def gather_bf16_k_cache_triton(
    out: torch.Tensor,
    ...
    assert k_cache.dtype == torch.bfloat16 and k_cache.stride(-1) == 1
    num_reqs = seq_lens.shape[0]
    NUM_WORKERS = 128
    _gather_bf16_k_kernel[(num_reqs, NUM_WORKERS)](
        out,
        out.stride(0),
        out.stride(1),
        k_cache,
...
def dequantize_and_gather_k_cache(
    ...
    A bf16 cache (plain 512-element rows, the A100 bf16 main-KV path) is a
    pure paged gather: the page already holds the workspace dtype.
    """
    if k_cache.dtype == torch.bfloat16:
        gather_bf16_k_cache_triton(
