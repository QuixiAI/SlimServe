...
"""Triton replacement for ``_C.fused_deepseek_v4_qnorm_rope_kv_rope_quant_insert``.

Does: Q per-head RMSNorm + GPT-J RoPE, KV GPT-J RoPE + UE8M0 FP8 quant + insert.
Uses the existing quantize_and_insert_k_cache for the FP8 portion, so the SWA
cache rows land in the same fp8_ds_mla layout the Triton compressor writes and
the sparse-MLA Triton decode/prefill kernels read.

Recovered from upstream vLLM's XPU DeepSeek-V4 tree (xpu_qnorm_rope_kv_fp8_insert)
for the Intel XPU platform of this fork; platform-neutral Triton.
"""
...
def triton_qnorm_rope_kv_fp8_insert(
    ...
    """Triton: qnorm+rope on Q (in place), rope on KV, then FP8 UE8M0 quant+insert."""
