...
"""Kimi K3's TikTokenTokenizer, vendored from the released model repo.

Carried verbatim rather than reimplemented. The tokenizer is the one
component where "close enough" is silently wrong: a different
pre-tokenizer split shifts every id after the divergence and the model
still produces fluent-looking output. `build_kimi_k3_tokenizer_from_gguf`
feeds it a vocab file materialised from the GGUF, so the ranks come from
the served file while the splitting logic stays byte-identical to the
reference.
"""
...
class TikTokenTokenizer(PreTrainedTokenizer):
    ...
    @staticmethod
    def _truncate(
        ids: list[int], truncation: bool = False, max_length: int | None = None
    ) -> list[int]:
