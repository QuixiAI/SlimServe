...
class ResponsesCustomToolCallParam(TypedDict):
    """Model-independent internal form of a Responses custom tool call.

    Chat Completions does not expose raw-input custom tools, but Responses
    conversations still pass through the shared chat rendering pipeline. Keep
    the raw input intact here and let each model renderer encode it in that
...
def custom_tool_as_function_dict(tool: dict[str, Any]) -> dict[str, Any]:
    """Adapt a Responses custom-tool declaration for ordinary chat templates."""
...
def custom_tool_call_as_function_dict(
    tool_call: ResponsesCustomToolCallParam,
) -> dict[str, Any]:
    """Adapt a raw custom-tool history item for ordinary chat templates."""
    return {
        "id": tool_call["id"],
        "type": "function",
        "function": {
...
def adapt_custom_tools_for_chat_template(
    tools: list[dict[str, Any]] | None,
) -> list[dict[str, Any]] | None:
    """Return function-shaped tools for chat templates without native support."""
    if tools is None:
        return None
    return [
        custom_tool_as_function_dict(tool) if tool.get("type") == "custom" else tool
        for tool in tools
    ]


def adapt_custom_tool_calls_for_chat_template(
    messages: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Copy and function-shape custom calls for a model's chat template."""
    adapted: list[dict[str, Any]] = []
    for message in messages:
        tool_calls = message.get("tool_calls")
        if not tool_calls or not any(
            isinstance(call, dict) and call.get("type") == "custom"
        ...
        copied = dict(message)
        copied["tool_calls"] = [
            custom_tool_call_as_function_dict(call)  # type: ignore[arg-type]
            if isinstance(call, dict) and call.get("type") == "custom"
            else call
            for call in tool_calls
        ]
...
class CustomChatCompletionMessageParam(TypedDict, total=False):
    ...
    tool_calls: (
        list[ChatCompletionMessageToolCallParam | ResponsesCustomToolCallParam] | None
    )
            ...
            for item in tool_calls:
                ...
                if item_type == "custom":
                    if not isinstance(item.get("name"), str) or not isinstance(
                        ...
                        raise VLLMValidationError(
                            "Responses custom tool calls require string "
                ...
                if item_type != "function" or not isinstance(function, dict):
                    raise VLLMValidationError(
