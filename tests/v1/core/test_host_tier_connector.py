...
def test_connector_shutdown_closes_nvme_backend_but_flushes_dma():
    ...
    conn._dma.reset_mock()
    conn.use_nvme = False
    conn.shutdown()
    conn._dma.flush.assert_called_once_with()
    conn._dma.shutdown.assert_not_called()
