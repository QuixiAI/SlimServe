...
def test_natural_closer_at_budget_boundary_preserves_main_sampling_behavior():
    drafts = [120, END, ord("{")]
    holder = make_holder(2, [120], drafts)
    logits = torch.zeros(3, VOCAB_SIZE)
    holder.apply_to_logits(logits, False, [drafts])
    # Current main deliberately lets an in-budget natural draft closer pass
    # through rejection sampling without forcing another closing marker.
