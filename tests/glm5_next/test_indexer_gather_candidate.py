...
def inputs(rows):
    return (
        ...
        torch.empty(rows, 512),
        512,
        576,
        128**-0.5,
        512,
        torch.empty(rows, 2080, dtype=torch.int32),
        4,
    )
...
def test_disabled_preserves_serving_fallback(monkeypatch, rows):
    ...
    assert all(
        actual is expected for actual, expected in zip(original.call_args.args, args)
    )
...
def test_owned_communicator_order_and_global_rows(monkeypatch, rows, rank, production):
    ...
    def topk(*call, **kwargs):  # the owned path also passes group=
        events.append("topk")
    ...
    group = SimpleNamespace(
        world_size=8, rank_in_group=rank, device_communicator=communicator
    )
    module = serving if production else candidate
    monkeypatch.setattr(
        distributed if production else candidate, "get_tp_group", lambda: group
    )
    monkeypatch.setattr(module, "_pooled_topk", topk)
    monkeypatch.setattr(module, "_expand_topk_kernel", Expand())
    selector = (
        serving._pooled_select if production else candidate.pooled_select_existing_tp
    )
...
def test_missing_communicator_fails_closed(monkeypatch, pynccl, production):
    group = SimpleNamespace(
        ...
        device_communicator=SimpleNamespace(
            wait_for_comm_init=lambda: None, pynccl_comm=pynccl
        ),
    )
    monkeypatch.setattr(
        distributed if production else candidate, "get_tp_group", lambda: group
