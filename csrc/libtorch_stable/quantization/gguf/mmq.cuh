...
// Same geometry as q4_0: identical block size and quants per int.
#if defined(USE_ROCM)
...
__launch_bounds__(WARP_SIZE_GGUF* NWARPS_MXFP4, 2)
#endif
    mul_mat_mxfp4(const void* __restrict__ vx, const void* __restrict__ vy,
                  scalar_t* __restrict__ dst, const int ncols_x,
                  const int nrows_x, const int ncols_y, const int nrows_y,
                  const int nrows_dst) {
  ...
  mul_mat_q<scalar_t, QK_MXFP4, QR_MXFP4, QI_MXFP4, true, block_mxfp4, mmq_x,
            mmq_y, nwarps, allocate_tiles_mxfp4<mmq_y>,
            load_tiles_mxfp4<mmq_y, nwarps, need_check>, VDR_MXFP4_Q8_1_MMQ,
            vec_dot_mxfp4_q8_1_mul_mat>(vx, vy, dst, ncols_x, nrows_x, ncols_y,
                                        nrows_y, nrows_dst);
}

template <typename scalar_t>
static void ggml_mul_mat_mxfp4_q8_1_cuda(const void* vx, const void* vy,
                                         scalar_t* dst, const int ncols_x,
                                         const int nrows_x, const int ncols_y,
                                         const int nrows_y, const int nrows_dst,
                                         cudaStream_t stream) {
  ...
  if (nrows_x % mmq_y == 0) {
    const bool need_check = false;
    mul_mat_mxfp4<scalar_t, need_check><<<block_nums, block_dims, 0, stream>>>(
        vx, vy, dst, ncols_x, nrows_x, ncols_y, nrows_y, nrows_dst);
  } else {
    const bool need_check = true;
    mul_mat_mxfp4<scalar_t, need_check><<<block_nums, block_dims, 0, stream>>>(
...
static void ggml_mul_mat_q5_1_q8_1_cuda(const void* vx, const void* vy,
  ...
  #define MMQ_Y_Q8_0_SMALL 64
  #define MMQ_Y_Q8_0_LARGE 128
  #define NWARPS_Q8_0 8
#else
  #define MMQ_X_Q8_0 4
  #define MMQ_Y_Q8_0_SMALL 32
  #define MMQ_Y_Q8_0_LARGE 32
  #define NWARPS_Q8_0 4
#endif

template <typename scalar_t, bool need_check, int mmq_y>
static __global__ void
#if defined(USE_ROCM)
...
#define VLLM_Q8_0_LAUNCH(MMQ_Y, NEED_CHECK)                                    \
  do {                                                                         \
    constexpr int tile_y = MMQ_Y;                                              \
    const int block_num_x = (nrows_x + tile_y - 1) / tile_y;                   \
    const int block_num_y = (ncols_y + mmq_x - 1) / mmq_x;                     \
    const dim3 block_nums(block_num_x, block_num_y, 1);                        \
    mul_mat_q8_0<scalar_t, NEED_CHECK, tile_y>                                 \
        <<<block_nums, block_dims, 0, stream>>>(vx, vy, dst, ncols_x, nrows_x, \
                                                ncols_y, nrows_y, nrows_dst);  \
  } while (0)

#if defined(USE_ROCM)
  const bool use_small_tile = int64_t(nrows_x) * ncols_y <= 2 * 1024 * 1024;
#else
  const bool use_small_tile = true;
#endif
  if (use_small_tile && nrows_x % MMQ_Y_Q8_0_SMALL == 0) {
    VLLM_Q8_0_LAUNCH(MMQ_Y_Q8_0_SMALL, false);
  } else if (use_small_tile) {
    VLLM_Q8_0_LAUNCH(MMQ_Y_Q8_0_SMALL, true);
  } else if (nrows_x % MMQ_Y_Q8_0_LARGE == 0) {
    VLLM_Q8_0_LAUNCH(MMQ_Y_Q8_0_LARGE, false);
  } else {
    VLLM_Q8_0_LAUNCH(MMQ_Y_Q8_0_LARGE, true);
  }
#undef VLLM_Q8_0_LAUNCH
}
