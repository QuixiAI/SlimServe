...
void launch_dsv4_indexer_token_merge_impl(
    torch::stable::Tensor& logits, const torch::stable::Tensor& lengths,
    const torch::stable::Tensor& local_indices, torch::stable::Tensor& output,
    int64_t k, int64_t logits_peer_data_ptr, int64_t indices_peer_data_ptr,
    int64_t lengths_peer_data_ptr, int64_t logits_byte_offset,
    int64_t indices_byte_offset, int64_t lengths_byte_offset,
    const std::array<int64_t, 8>& signal_ptrs, int64_t self_signal_ptr,
...
// The DSV4 multi-head-channel all-reduce family is Ampere-only: it is
// built on tms::dsv4_mhc / vllm::dsv4_mhc_channel_owned, which
// custom_all_reduce.cuh only includes under `#if !defined(USE_ROCM)`.
// This translation unit is in the shared source list, so it needs the
// same guard the header has -- without it the ROCm build fails outright
// (undeclared tms::dsv4_mhc, no member allreduce_dsv4_mhc, ...).
#if !defined(USE_ROCM)
__global__ void dsv4_add_local_partials(nv_bfloat16* output,
...
void all_reduce_dsv4_mhc(
    ...
    const std::optional<torch::stable::Tensor>& quant_input, double rms_eps,
    double pre_eps, double sinkhorn_eps, double post_multiplier,
    int64_t sinkhorn_repeat, double norm_eps, bool input_prepared,
    bool own_projections, bool publish_prepared, fptr_t _reg_buffer,
    int64_t reg_buffer_sz_bytes) {
  auto fa = reinterpret_cast<vllm::CustomAllreduce*>(_fa);
  const torch::stable::accelerator::DeviceGuard device_guard(
      inp.get_device_index());
  ...
  if (addend) {
    ...
    STD_TORCH_CHECK(
        _is_weak_contiguous(const_cast<torch::stable::Tensor&>(*addend)));
  }
  ...
  STD_TORCH_CHECK(comb_mix.scalar_type() ==
                  torch::headeronly::ScalarType::Float);
  STD_TORCH_CHECK(fn.scalar_type() == torch::headeronly::ScalarType::Float ||
  ...
  STD_TORCH_CHECK(base.scalar_type() == torch::headeronly::ScalarType::Float);
  ...
  const bool local_input_owned = layer_input.dim() == 2 &&
                                 layer_input.size(0) == 1 &&
                                 layer_input.size(1) == local_hidden;
  STD_TORCH_CHECK(layer_input.dim() == 2 && layer_input.size(0) == 1 &&
  ...
  STD_TORCH_CHECK(
      !local_input_owned || (norm_weight.has_value() && own_projections),
  ...
  if (reg_buffer) {
    ...
    if (addend) {
      ...
      STD_CUDA_CHECK(cudaMemcpyAsync(reg_buffer, inp.const_data_ptr(),
  ...
  const auto* norm_weight_ptr =
      norm_weight
          ...
          : nullptr;
  auto* quant_input_ptr =
      quant_input ? reinterpret_cast<tms::dsv4_mhc::block_q8_1*>(
...
void dsv4_channel_owned_mhc(
  ...
  STD_TORCH_CHECK(
      fa->world_size_ == 2 || fa->world_size_ == 4 || fa->world_size_ == 8,
      "DSV4 channel ownership requires TP2, TP4, or TP8");
  const int64_t local_hidden = 4096 / fa->world_size_;
  const bool reduce_input =
      inp.dim() == 2 && inp.size(0) == 1 && inp.size(1) == 4096;
  const torch::stable::accelerator::DeviceGuard device_guard(
      inp.get_device_index());
  const cudaStream_t stream = get_current_cuda_stream(inp.get_device_index());
  ...
  STD_TORCH_CHECK(
      partial.scalar_type() == torch::headeronly::ScalarType::Float &&
          partial.numel() >= vllm::dsv4_mhc_channel_owned::kPartialValues &&
  ...
  if (shared_partial != nullptr) {
    STD_TORCH_CHECK(vllm::dsv4_mhc_channel_owned::kPartialValues *
                        sizeof(float) <=
                    reg_buffer_sz_bytes);
  } else {
  ...
  if (reduce_input) {
    ...
    input_peers =
        fa->resolve_rank_data(stream, const_cast<void*>(inp.const_data_ptr()));
    if (addend) {
  ...
  if (reduce_input) {
    ...
    owned_input =
        reinterpret_cast<const nv_bfloat16*>(layer_input.const_data_ptr());
  }
  ...
  if (use_monolithic) {
    #define LAUNCH_CHANNEL_OWNED_MONOLITHIC(FN_T, NGPU)                      \
      do {                                                                   \
        auto kernel =                                                        \
            vllm::dsv4_mhc_channel_owned::post_pre_monolithic<FN_T, NGPU>;   \
        ...
        nv_bfloat16* kernel_residual_out =                                   \
            reinterpret_cast<nv_bfloat16*>(residual_out.mutable_data_ptr()); \
        float* kernel_debug =                                                \
        ...
        nv_bfloat16* kernel_layer_input =                                    \
            reinterpret_cast<nv_bfloat16*>(layer_input.mutable_data_ptr());  \
        ...
        void* kernel_args[] = {&partial_peers,                               \
                               &fa->sg_,                                     \
                               &fa->self_sg_,                                \
                               &kernel_input,                                \
                               &kernel_residual,                             \
                               ...
                               &kernel_pre_eps,                              \
                               &kernel_sinkhorn_eps,                         \
                               &kernel_post_multiplier,                      \
                               &kernel_sinkhorn_repeat,                      \
                               &kernel_norm_eps,                             \
                               &fa->rank_};                                  \
  ...
  #define LAUNCH_CHANNEL_OWNED(FN_T, NGPU)                                     \
    do {                                                                       \
      ...
      const nv_bfloat16* urgent_residual =                                     \
          reinterpret_cast<const nv_bfloat16*>(residual.const_data_ptr());     \
      const float* urgent_post =                                               \
          reinterpret_cast<const float*>(post_mix.const_data_ptr());           \
      const float* urgent_comb =                                               \
          reinterpret_cast<const float*>(comb_mix.const_data_ptr());           \
      const FN_T* urgent_fn =                                                  \
          reinterpret_cast<const FN_T*>(fn.const_data_ptr());                  \
      nv_bfloat16* urgent_residual_out =                                       \
          reinterpret_cast<nv_bfloat16*>(residual_out.mutable_data_ptr());     \
      float* urgent_debug =                                                    \
          reinterpret_cast<float*>(partial.mutable_data_ptr());                \
      ...
      const float* urgent_base =                                               \
          reinterpret_cast<const float*>(base.const_data_ptr());               \
      nv_bfloat16* urgent_layer_input =                                        \
          reinterpret_cast<nv_bfloat16*>(layer_input.mutable_data_ptr());      \
      ...
      void* urgent_args[] = {&partial_peers,       &fa->sg_,                   \
                             &fa->self_sg_,        &urgent_input,              \
                             &urgent_residual,     &urgent_post,               \
                             &urgent_comb,         &urgent_fn,                 \
                             &urgent_residual_out, &partial_ptr,               \
                             &urgent_debug,        &urgent_scale,              \
                             &urgent_base,         &urgent_layer_input,        \
                             &urgent_norm_weight,  &quant_ptr,                 \
                             &urgent_rms_eps,      &urgent_pre_eps,            \
                             &urgent_norm_eps,     &fa->rank_};                \
      CUDACHECK(cudaLaunchCooperativeKernel(                                   \
          reinterpret_cast<const void*>(urgent_kernel),                        \
          dim3(vllm::dsv4_mhc_channel_owned::kUrgentSplits),                   \
          dim3(vllm::dsv4_mhc_channel_owned::kThreads), urgent_args, 0,        \
          stream));                                                            \
      ...
      const nv_bfloat16* deferred_residual =                                   \
          reinterpret_cast<const nv_bfloat16*>(residual_out.const_data_ptr()); \
      ...
      void* deferred_args[] = {&partial_peers,                                 \
                               &fa->sg_,                                       \
                               &fa->self_sg_,                                  \
                               &deferred_residual,                             \
                               &deferred_fn,                                   \
                               ...
                               &deferred_post,                                 \
                               &deferred_comb,                                 \
                               &deferred_sinkhorn_eps,                         \
                               &deferred_post_multiplier,                      \
                               &deferred_sinkhorn_repeat,                      \
                               &fa->rank_};                                    \
      CUDACHECK(cudaLaunchCooperativeKernel(                                   \
...
void dsv4_owned_attention_projections(
    ...
    const torch::stable::Tensor& bf16_weight2, torch::stable::Tensor& q8_output,
    torch::stable::Tensor& bf16_output0, torch::stable::Tensor& bf16_output1,
    torch::stable::Tensor& bf16_output2, torch::stable::Tensor& partial,
    torch::stable::Tensor& reduced, fptr_t reg_buffer_ptr,
    int64_t reg_buffer_sz_bytes) {
  auto* fa = reinterpret_cast<vllm::CustomAllreduce*>(_fa);
  ...
  STD_TORCH_CHECK(q8_output.scalar_type() == bf16 &&
                  q8_output.numel() == q8_rows && q8_output.is_contiguous());
  ...
  vllm::dsv4_tp_input_owned::launch_attention_finalize(
      reduced_ptr, reinterpret_cast<nv_bfloat16*>(q8_output.mutable_data_ptr()),
      reinterpret_cast<float*>(bf16_output0.mutable_data_ptr()),
      reinterpret_cast<nv_bfloat16*>(bf16_output1.mutable_data_ptr()),
      reinterpret_cast<float*>(bf16_output2.mutable_data_ptr()), q8_rows, rows0,
      rows1, rows2, stream);
  STD_CUDA_CHECK(cudaGetLastError());
}

void dsv4_gather_owned_q8(fptr_t _fa, const torch::stable::Tensor& local_quant,
                          torch::stable::Tensor& full_quant,
                          fptr_t reg_buffer_ptr, int64_t reg_buffer_sz_bytes) {
  auto* fa = reinterpret_cast<vllm::CustomAllreduce*>(_fa);
  STD_TORCH_CHECK(
      fa != nullptr && (fa->world_size_ == 2 || fa->world_size_ == 4 ||
                        fa->world_size_ == 8),
  ...
  if (published != nullptr) {
    ...
    STD_CUDA_CHECK(cudaMemcpyAsync(published, local_quant.const_data_ptr(),
...
void dsv4_gather_owned_bf16(fptr_t _fa,
                            const torch::stable::Tensor& local_input,
                            torch::stable::Tensor& full_output,
                            fptr_t reg_buffer_ptr,
                            int64_t reg_buffer_sz_bytes) {
  auto* fa = reinterpret_cast<vllm::CustomAllreduce*>(_fa);
  STD_TORCH_CHECK(
      fa != nullptr && (fa->world_size_ == 2 || fa->world_size_ == 4 ||
  ...
  STD_TORCH_CHECK(full_output.scalar_type() == local_input.scalar_type() &&
                      full_output.numel() == rows * 4096 &&
                      full_output.is_contiguous(),
  ...
  if (published != nullptr) {
    ...
    STD_TORCH_CHECK(bytes <= reg_buffer_sz_bytes);
    STD_CUDA_CHECK(cudaMemcpyAsync(published, local_input.const_data_ptr(),
                                   bytes, cudaMemcpyDeviceToDevice, stream));
  } else {
...
void dsv4_owned_router(fptr_t _fa, const torch::stable::Tensor& local_input,
  ...
  STD_TORCH_CHECK(
      ...
      local_input.size(1) == local_hidden && local_input.is_contiguous());
  ...
  fa->allreduce<float>(stream, partial_ptr,
                       reinterpret_cast<float*>(output.mutable_data_ptr()),
                       rows);
  STD_CUDA_CHECK(cudaGetLastError());
}

void dsv4_channel_owned_q2_down(fptr_t _fa,
                                const torch::stable::Tensor& quant_mid,
  ...
  const int64_t quant_bytes = quant_mid.numel() * quant_mid.element_size();
    ...
    do {                                                                  \
      ...
      vllm::dsv4_mhc_channel_owned::channel_q2_down<NGPU, TOP_K, LOCAL_K> \
          <<<(local_rows + 7) / 8, 256, 0, stream>>>(                     \
              ...
              weights.stride(0), local_rows,                              \
              static_cast<int>(weights.size(0)));                         \
...
void dsv4_channel_owned_q2_down_pending(fptr_t _fa,
                                        const torch::stable::Tensor& pending,
                                        const torch::stable::Tensor& addend,
                                        torch::stable::Tensor& scratch,
                                        torch::stable::Tensor& output,
                                        fptr_t reg_buffer_ptr,
                                        int64_t reg_buffer_sz_bytes) {
  auto* fa = reinterpret_cast<vllm::CustomAllreduce*>(_fa);
  ...
  const cudaStream_t stream =
      get_current_cuda_stream(pending.get_device_index());
  ...
  STD_TORCH_CHECK(
      scratch.scalar_type() == torch::headeronly::ScalarType::Byte &&
          scratch.is_contiguous() && scratch.numel() >= required_scratch_bytes,
  ...
  const int64_t full_quant_bytes = 6 * (2048 / QK8_1) * sizeof(block_q8_1);
  ...
  STD_TORCH_CHECK(publication != nullptr &&
  ...
  auto* header = reinterpret_cast<const vllm::dsv4_q2_mhc_ar::PendingQ2Header*>(
      static_cast<const uint8_t*>(pending.const_data_ptr()) + pending_bytes -
      sizeof(vllm::dsv4_q2_mhc_ar::PendingQ2Header));
  auto* gathered_quant =
      reinterpret_cast<block_q8_1*>(scratch.mutable_data_ptr());
  auto* gathered_addend = reinterpret_cast<float*>(
      static_cast<uint8_t*>(scratch.mutable_data_ptr()) + gathered_quant_bytes);
  ...
  #define LAUNCH_PENDING_CHANNEL_Q2(NGPU, LOCAL_K, GATHER_ADDEND, PUSH_ADDEND, \
                                    PUSH_QUANT)                                \
    do {                                                                       \
      vllm::dsv4_mhc_channel_owned::channel_q2_gather<                         \
          NGPU, 6, LOCAL_K, GATHER_ADDEND, PUSH_ADDEND, PUSH_QUANT>            \
          <<<1, 512, 0, stream>>>(                                             \
      ...
      vllm::dsv4_mhc_channel_owned::channel_q2_down<NGPU, 6, LOCAL_K, true,    \
                                                    PUSH_QUANT>                \
          <<<(local_rows + 7) / 8, 256, 0, stream>>>(                          \
              nullptr, publication_peers, nullptr,                             \
              reinterpret_cast<nv_bfloat16*>(output.mutable_data_ptr()), 0,    \
              local_rows, 0, header, nullptr, 0, fa->rank_,                    \
...
void dsv4_channel_owned_moe(
    ...
    const torch::stable::Tensor& topk_ids, const torch::stable::Tensor& addend,
    torch::stable::Tensor& scratch, torch::stable::Tensor& output,
    double swiglu_limit, fptr_t reg_buffer_ptr, int64_t reg_buffer_sz_bytes) {
  auto* fa = reinterpret_cast<vllm::CustomAllreduce*>(_fa);
  STD_TORCH_CHECK(fa != nullptr, "DSV4 channel-owned MoE communicator is null");
  STD_TORCH_CHECK(fa->world_size_ == 2 || fa->world_size_ == 4,
  ...
  STD_TORCH_CHECK(
      w2.scalar_type() == torch::headeronly::ScalarType::Byte &&
          ...
          w2.size(2) == 2048 / QK_K * sizeof(block_q2_K) && w2.is_contiguous(),
      "DSV4 channel-owned W2 must be repacked uint8[E,4096/TP,672]");
  STD_TORCH_CHECK(
  ...
  STD_TORCH_CHECK(
      addend.scalar_type() == torch::headeronly::ScalarType::BFloat16 &&
          addend.dim() == 2 && addend.size(0) == 1 && addend.size(1) == 4096 &&
          addend.is_contiguous(),
      "DSV4 channel-owned shared addend must be BF16[1,4096]");
  STD_TORCH_CHECK(
      scratch.scalar_type() == torch::headeronly::ScalarType::Byte &&
  ...
  constexpr int64_t full_quant_bytes = 6 * (2048 / QK8_1) * sizeof(block_q8_1);
  const int64_t publication_slot_bytes =
  ...
  auto* addend_peers =
      direct_addend ? fa->resolve_rank_data(
                          stream, const_cast<void*>(addend.const_data_ptr()))
                    : nullptr;

  slimserve::dsv4_ampere::launch_iq2_xxs_gate_up_swiglu_q8_1_decode_publish(
      ...
      w1.stride(0), 4096, local_intermediate, static_cast<int>(w1.size(0)),
      static_cast<float>(swiglu_limit), true, stream);

  #define LAUNCH_PRODUCER_OWNED_MOE(NGPU, LOCAL_K, OWNERSHIP_THREADS,          \
                                    DIRECT_ADDEND)                             \
    do {                                                                       \
      vllm::dsv4_mhc_channel_owned::channel_q2_gather<                         \
          NGPU, 6, LOCAL_K, false, !DIRECT_ADDEND, false, true, DIRECT_ADDEND> \
          <<<1, OWNERSHIP_THREADS, 0, stream>>>(                               \
              nullptr,                                                         \
              reinterpret_cast<const nv_bfloat16*>(addend.const_data_ptr()),   \
              publication_peers, nullptr, nullptr, fa->sg_, fa->self_sg_,      \
              local_rows, fa->rank_);                                          \
      vllm::dsv4_mhc_channel_owned::channel_q2_down<NGPU, 6, LOCAL_K, false,   \
                                                    true, true>                \
          <<<(local_rows + 7) / 8, 256, 0, stream>>>(                          \
...
void dsv4_output_owned_moe(fptr_t _fa, const torch::stable::Tensor& quant_input,
                           const torch::stable::Tensor& w1,
                           ...
                           fptr_t reg_buffer_ptr, int64_t reg_buffer_sz_bytes) {
  auto* fa = reinterpret_cast<vllm::CustomAllreduce*>(_fa);
  STD_TORCH_CHECK(fa != nullptr, "DSV4 output-owned MoE communicator is null");
  STD_TORCH_CHECK(fa->world_size_ == 2 || fa->world_size_ == 4,
  ...
  STD_TORCH_CHECK(w2.scalar_type() == torch::headeronly::ScalarType::Byte &&
                      w2.dim() == 3 && w2.size(0) == w1.size(0) &&
                      w2.size(1) == local_rows &&
  ...
  #define LAUNCH_OUTPUT_OWNED_MOE(NGPU, LOCAL_K)                               \
    do {                                                                       \
      vllm::dsv4_mhc_channel_owned::channel_shared_q8_publish<                 \
          NGPU, 6, LOCAL_K><<<1, 64, 0, stream>>>(                             \
          reinterpret_cast<const block_q8_1*>(shared_quant.const_data_ptr()),  \
        ...
        CUDACHECK(                                                             \
            (vllm::dsv4_mhc_channel_owned::                                    \
                 launch_channel_shared_gate_up_publish<NGPU, 6, LOCAL_K>(      \
                     shared_w1.const_data_ptr(), quant_input.const_data_ptr(), \
                     reinterpret_cast<nv_bfloat16*>(                           \
                         shared_scratch.mutable_data_ptr()),                   \
                     publication_peers, fa->sg_, fa->self_sg_, 4096 / QK8_0,   \
                     static_cast<float>(swiglu_limit), fa->rank_, stream)));   \
        vllm::dsv4_mhc_channel_owned::channel_q2_down<NGPU, 6, LOCAL_K, false, \
                                                      true, false, true>       \
            <<<(local_rows + 7) / 8, 256, 0, stream>>>(                        \
                reinterpret_cast<const uint8_t*>(w2.const_data_ptr()),         \
                ...
                reinterpret_cast<const int*>(topk_ids.const_data_ptr()),       \
                reinterpret_cast<nv_bfloat16*>(output.mutable_data_ptr()),     \
                w2.stride(0), local_rows, static_cast<int>(w2.size(0)),        \
                nullptr, nullptr, 0, fa->rank_, nullptr, nullptr,              \
                fa->self_sg_,                                                  \
                reinterpret_cast<const uint8_t*>(shared_w2.const_data_ptr())); \
      } while (0)
...
void dsv4_output_owned_q8(fptr_t _fa, const torch::stable::Tensor& local_quant,
                          const torch::stable::Tensor& aligned_weight,
                          torch::stable::Tensor& output, int64_t rows_per_cta,
                          fptr_t reg_buffer_ptr, int64_t reg_buffer_sz_bytes) {
  auto* fa = reinterpret_cast<vllm::CustomAllreduce*>(_fa);
  STD_TORCH_CHECK(fa != nullptr, "DSV4 output-owned Q8 communicator is null");
  STD_TORCH_CHECK(
      fa->world_size_ == 2 || fa->world_size_ == 4 || fa->world_size_ == 8,
  ...
  STD_TORCH_CHECK(
      local_quant.scalar_type() == torch::headeronly::ScalarType::Int &&
          ...
          local_quant.size(0) <= vllm::dsv4_tp_output_owned::kMaxTokens &&
          local_quant.size(1) % vllm::dsv4_tp_output_owned::kQ8Words == 0,
  ...
  STD_TORCH_CHECK(
      full_blocks > 0 &&
          full_blocks <= vllm::dsv4_tp_output_owned::kMaxFullColumns /
                             vllm::dsv4_tp_output_owned::kQ8Block,
      "DSV4 output-owned Q8 supports at most 8192 full input columns");
  const int local_rows = 4096 / fa->world_size_;
  STD_TORCH_CHECK(
      aligned_weight.scalar_type() == torch::headeronly::ScalarType::Byte &&
      ...
      "DSV4 output-owned Q8 weight must be aligned uint8 [4096/TP, "
      "full_K/32*34]");
  STD_TORCH_CHECK(
      output.scalar_type() == torch::headeronly::ScalarType::BFloat16 &&
  ...
  STD_TORCH_CHECK(rows_per_cta == 1 || rows_per_cta == 2 || rows_per_cta == 4,
                  "DSV4 output-owned Q8 rows_per_cta must be 1, 2, or 4");
  STD_TORCH_CHECK(
      reg_buffer_ptr != 0 &&
          2 * vllm::dsv4_tp_output_owned::kSlotBytes <= reg_buffer_sz_bytes,
...
void dsv4_owned_reduce_scatter(
  ...
  STD_TORCH_CHECK(
      input.scalar_type() == torch::headeronly::ScalarType::BFloat16 &&
          input.dim() == 2 && input.is_contiguous() && input.size(0) >= 1 &&
  ...
  if (direct) {
    ...
    STD_TORCH_CHECK(
        2 * vllm::dsv4_tp_reduce_scatter::kSlotBytes <= reg_buffer_sz_bytes,
...
void all_reduce_dsv4_q2_mhc(
    ...
    const torch::stable::Tensor& addend, torch::stable::Tensor& producer_output,
    const torch::stable::Tensor& residual,
    const torch::stable::Tensor& post_mix,
    const torch::stable::Tensor& comb_mix, const torch::stable::Tensor& fn,
    torch::stable::Tensor& residual_out, torch::stable::Tensor& partial,
  ...
  STD_TORCH_CHECK(addend.scalar_type() == pending.scalar_type() &&
                      addend.sizes() == pending.sizes() &&
                      addend.is_contiguous(),
                  "DSV4 pending Q2_K addend must match pending input");
  STD_TORCH_CHECK(producer_output.scalar_type() == pending.scalar_type() &&
                      producer_output.sizes() == pending.sizes() &&
                      producer_output.is_contiguous(),
                  "DSV4 Q2_K producer output must be contiguous BF16[1,4096]");
  STD_TORCH_CHECK(residual.scalar_type() == pending.scalar_type() &&
  ...
  STD_TORCH_CHECK(
      partial.scalar_type() == torch::headeronly::ScalarType::Float &&
          partial.numel() >= vllm::dsv4_q2_mhc_ar::kRequiredPartialValues &&
          partial.is_contiguous(),
...
#endif  // !defined(USE_ROCM) -- DSV4 mHC all-reduce family

void dsv4_indexer_peer_topk(fptr_t _fa, torch::stable::Tensor& logits,
                            const torch::stable::Tensor& lengths,
                            torch::stable::Tensor& output,
                            torch::stable::Tensor& workspace, int64_t k,
                            int64_t max_seq_len, fptr_t reg_buffer_ptr,
                            int64_t reg_buffer_sz_bytes) {
  auto* fa = reinterpret_cast<vllm::CustomAllreduce*>(_fa);
  ...
  if (reg_buffer_ptr == 0) {
    output_peer_data = fa->resolve_rank_data(stream, output.mutable_data_ptr());
  }
...
void dsv4_indexer_token_merge(fptr_t _fa, torch::stable::Tensor& logits,
                              const torch::stable::Tensor& lengths,
  ...
  if (shared_input != nullptr) {
    ...
    STD_CUDA_CHECK(cudaMemcpyAsync(base, logits.const_data_ptr(), logits_bytes,
                                   cudaMemcpyDeviceToDevice, stream));
    STD_CUDA_CHECK(
        cudaMemcpyAsync(base + indices_offset, local_indices.const_data_ptr(),
                        indices_bytes, cudaMemcpyDeviceToDevice, stream));
