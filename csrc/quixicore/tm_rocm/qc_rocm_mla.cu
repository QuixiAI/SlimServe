...
void mla_decode_fwd(torch::Tensor q, torch::Tensor kv_cache,
  ...
  CK(seq_lens);
  CK(out);
  TORCH_CHECK(q.dim() == 3 && q.size(2) == kEntry,
              "q must be [B, H, ", kEntry, "]");
  TORCH_CHECK(out.dim() == 3 && out.size(2) == kLatent,
              "out must be [B, H, ", kLatent, "]");
  ...
  auto partial_out =
      torch::empty({batch, num_heads, num_splits, kLatent}, f32);
