...
static const half* hp(const torch::Tensor& t) {
  return reinterpret_cast<const half*>(t.data_ptr());
}
static half* hpm(torch::Tensor& t) {
  return reinterpret_cast<half*>(t.data_ptr());
}
static const __nv_bfloat16* bp(const torch::Tensor& t) {
  return reinterpret_cast<const __nv_bfloat16*>(t.data_ptr());
}
static __nv_bfloat16* bpm(torch::Tensor& t) {
...
static torch::Tensor py_dsv4_router_gemm(torch::Tensor x,
  ...
  TORCH_CHECK(
      x.size(0) >= 1 && x.size(0) <= 8 && x.size(1) == dsv4_router::HIDDEN,
  ...
  auto output = torch::empty({x.size(0), dsv4_router::EXPERTS},
                             x.options().dtype(torch::kFloat));
  dsv4_router::launch(bp(x), bp(weight), fpm(output), int(x.size(0)), stream());
  return output;
}
...
static std::tuple<torch::Tensor, torch::Tensor> py_dsv4_hash_router(
  ...
  TORCH_CHECK(
      (input_ids.scalar_type() == torch::kInt ||
  ...
  TORCH_CHECK(x.dim() == 2 && weight.dim() == 2 && input_ids.dim() == 1 &&
                  tid2eid.dim() == 2,
  ...
  auto topk_weights = torch::empty({x.size(0), dsv4_router::HASH_TOPK},
                                   x.options().dtype(torch::kFloat));
  auto topk_ids = torch::empty({x.size(0), dsv4_router::HASH_TOPK},
  ...
  if (input_ids.scalar_type() == torch::kInt) {
    dsv4_router::launch_hash(
        ...
        tid2eid.data_ptr<int32_t>(), padding_ptr, float(routed_scaling_factor),
        fpm(topk_weights), topk_ids.data_ptr<int32_t>(), int(x.size(0)),
...
static torch::Tensor py_dsv4_projection_gemv(torch::Tensor x,
  ...
  TORCH_CHECK(
      x.size(0) >= 1 && x.size(0) <= 8 && x.size(1) == dsv4_projection::HIDDEN,
  ...
  auto output = torch::empty(
      ...
      x.options().dtype(bf16_output ? torch::kBFloat16 : torch::kFloat));
  if (bf16_output) {
...
__global__ void fill_short_context_topk_indices_kernel(
    int* __restrict__ output, const int64_t* __restrict__ positions, int rows,
...
static void py_fill_short_context_topk_indices(torch::Tensor output,
                                               torch::Tensor positions,
                                               int64_t topk,
                                               int64_t compress_ratio) {
  CK(output);
  CK(positions);
  TORCH_CHECK(output.scalar_type() == torch::kInt, "output must be int32");
  ...
  if (rows > 0) {
    fill_short_context_topk_indices_kernel<<<rows, 256, 0, stream()>>>(
        output.data_ptr<int>(), positions.data_ptr<int64_t>(), rows, int(topk),
        int(compress_ratio));
  }
}
...
static void launch_dsv4_mhc_pre_transition(
  ...
  __nv_bfloat16* residual_out_ptr = FUSED_POST ? bpm(*residual_out) : nullptr;
  ...
  auto kernel =
      dsv4_mhc::fused_pre_transition<FUSED_POST, RMS_NORM, 4096, NSPLITS, FnT>;
  void* args[] = {
      &x_ptr,           &residual_ptr,     &post_ptr,      &comb_ptr,
      &fn_ptr,          &residual_out_ptr, &partial_ptr,   &scale_ptr,
      &base_ptr,        &next_post_ptr,    &next_comb_ptr, &layer_input_ptr,
      &norm_ptr,        &rms_eps,          &pre_eps,       &sinkhorn_eps,
  ...
  TORCH_CHECK(error == cudaSuccess, "DSV4 cooperative mHC launch failed: ",
...
#define LAUNCH_MHC_TYPED(FN_T, NSPLITS)                                        \
  launch_dsv4_mhc_pre_transition<FN_T, FUSED_POST, RMS_NORM, NSPLITS>(         \
      x, residual, post_mix, comb_mix, fn, residual_out, partial, scale, base, \
      next_post, next_comb, layer_input, norm_weight, rms_eps, pre_eps,        \
...
static void launch_dsv4_mhc_partials(
    const __nv_bfloat16* x, const __nv_bfloat16* residual, const float* post,
    const float* comb, torch::Tensor fn, __nv_bfloat16* residual_out,
    float* partial, int hidden_size, dim3 grid) {
  if (fn.scalar_type() == torch::kHalf) {
    dsv4_mhc::partials<NOUT, FUSED_POST, half>
        <<<grid, dsv4_mhc::THREADS, 0, stream()>>>(
            x, residual, post, comb,
            reinterpret_cast<const half*>(fn.data_ptr()), residual_out, partial,
            hidden_size);
  } else {
    dsv4_mhc::partials<NOUT, FUSED_POST, float>
        <<<grid, dsv4_mhc::THREADS, 0, stream()>>>(x, residual, post, comb,
                                                   fp(fn), residual_out,
                                                   partial, hidden_size);
  }
}
...
static std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> py_dsv4_mhc_pre(
    ...
    torch::Tensor hc_base, double rms_eps, double pre_eps, double sinkhorn_eps,
  ...
  if (dsv4_mhc_cooperative_enabled() && T == 1 && H == 4096) {
    if (norm_weight) {
      launch_dsv4_mhc_pre_transition_selected<false, true>(
          nullptr, residual, nullptr, nullptr, fn, nullptr, partial, hc_scale,
          hc_base, post, comb, layer_input, &*norm_weight, float(rms_eps),
      ...
      launch_dsv4_mhc_pre_transition_selected<false, false>(
          nullptr, residual, nullptr, nullptr, fn, nullptr, partial, hc_scale,
          hc_base, post, comb, layer_input, nullptr, float(rms_eps),
          float(pre_eps), float(sinkhorn_eps), float(post_multiplier),
          int(sinkhorn_repeat), float(norm_eps));
    }
  ...
  dsv4_mhc::finalize_pre_mix<<<T, 32, 0, stream()>>>(
      fpm(partial), fp(hc_scale), fp(hc_base), fpm(post), fpm(comb), H,
  ...
  if (norm_weight) {
    ...
    dsv4_mhc::apply_pre_mix<dsv4_mhc::MIXES + 1>
        <<<dim3((H + dsv4_mhc::THREADS - 1) / dsv4_mhc::THREADS, T),
           dsv4_mhc::THREADS, 0, stream()>>>(fp(partial), bp(residual),
                                             bpm(layer_input), H);
  }
  return {post, comb, layer_input};
}
...
py_dsv4_mhc_fused_post_pre(torch::Tensor x, torch::Tensor residual,
                           torch::Tensor post_mix, torch::Tensor comb_mix,
                           torch::Tensor fn, torch::Tensor hc_scale,
                           torch::Tensor hc_base, double rms_eps,
  ...
  auto next_comb = torch::empty({T, dsv4_mhc::HC, dsv4_mhc::HC}, float_options);
  auto layer_input = torch::empty({T, H}, residual.options());
  if (dsv4_mhc_cooperative_enabled() && T == 1 && H == 4096) {
    if (norm_weight) {
      launch_dsv4_mhc_pre_transition_selected<true, true>(
          &x, residual, &post_mix, &comb_mix, fn, &residual_out, partial,
          hc_scale, hc_base, next_post, next_comb, layer_input, &*norm_weight,
          float(rms_eps), float(pre_eps), float(sinkhorn_eps),
          float(post_multiplier), int(sinkhorn_repeat), float(norm_eps));
    } else {
      launch_dsv4_mhc_pre_transition_selected<true, false>(
          &x, residual, &post_mix, &comb_mix, fn, &residual_out, partial,
          hc_scale, hc_base, next_post, next_comb, layer_input, nullptr,
          float(rms_eps), float(pre_eps), float(sinkhorn_eps),
          float(post_multiplier), int(sinkhorn_repeat), float(norm_eps));
    }
  ...
  launch_dsv4_mhc_partials<dsv4_mhc::MIXES, true>(
      bp(x), bp(residual), fp(post_mix), fp(comb_mix), fn, bpm(residual_out),
      fpm(partial), H, dim3(dsv4_mhc::SPLITS, T));
  dsv4_mhc::finalize_pre_mix<<<T, 32, 0, stream()>>>(
      fpm(partial), fp(hc_scale), fp(hc_base), fpm(next_post), fpm(next_comb),
  ...
  if (norm_weight) {
    ...
    dsv4_mhc::apply_pre_mix_rms_norm<dsv4_mhc::MIXES + 1>
        <<<T, dsv4_mhc::THREADS, 0, stream()>>>(
            fp(partial), bp(residual_out), bp(*norm_weight), bpm(layer_input),
            float(norm_eps));
  } else {
    dsv4_mhc::apply_pre_mix<dsv4_mhc::MIXES + 1>
        <<<dim3((H + dsv4_mhc::THREADS - 1) / dsv4_mhc::THREADS, T),
           dsv4_mhc::THREADS, 0, stream()>>>(fp(partial), bp(residual_out),
                                             bpm(layer_input), H);
  }
  return {residual_out, next_post, next_comb, layer_input};
}

static torch::Tensor py_dsv4_mhc_post(torch::Tensor x, torch::Tensor residual,
...
static torch::Tensor py_dsv4_hc_head(torch::Tensor residual, torch::Tensor fn,
  ...
  auto partial = torch::empty({T, dsv4_mhc::SPLITS, dsv4_mhc::HC + 1},
  ...
  dsv4_mhc::finalize_head_mix<<<T, 32, 0, stream()>>>(
      fpm(partial), fp(hc_scale), fp(hc_base), H, float(rms_eps),
      float(hc_eps));
...
// ---- kv cache management ----
static void py_kv_scatter(torch::Tensor key, torch::Tensor value,
                          torch::Tensor slot_mapping, torch::Tensor key_cache,
                          torch::Tensor value_cache, int64_t block_size) {
  CK(key);
  CK(value);
  CK(slot_mapping);
  CK(key_cache);
  ...
  kv_cache_scatter<half><<<T, 128, 0, stream()>>>(
      hp(key), hp(value), slot_mapping.data_ptr<int64_t>(), hpm(key_cache),
      hpm(value_cache), H, D, int(block_size));
}
static std::tuple<torch::Tensor, torch::Tensor> py_kv_gather(
    torch::Tensor key_cache, torch::Tensor value_cache,
    torch::Tensor block_table, torch::Tensor cu_seq_lens, int64_t num_tokens,
    int64_t H, int64_t D, int64_t block_size) {
  CK(key_cache);
  CK(value_cache);
  CK(block_table);
  CK(cu_seq_lens);
  ...
  kv_cache_gather<half><<<int(num_tokens), 128, 0, stream()>>>(
      hp(key_cache), hp(value_cache), hpm(k), hpm(v),
      block_table.data_ptr<int>(), cu_seq_lens.data_ptr<int>(), int(num_tokens),
      B, int(block_size), int(block_table.size(1)), int(H), int(D));
  return {k, v};
}
static void py_copy_blocks(torch::Tensor ks, torch::Tensor vs, torch::Tensor kd,
                           torch::Tensor vd, torch::Tensor pairs,
                           int64_t block_elems) {
  CK(ks);
  CK(vs);
  CK(kd);
  ...
  kv_cache_copy_blocks<half>
      <<<n, 128, 0, stream()>>>(hp(ks), hp(vs), hpm(kd), hpm(vd),
                                pairs.data_ptr<int64_t>(), int(block_elems));
}

// ---- decode attention ----
static torch::Tensor py_paged_attention(torch::Tensor q, torch::Tensor kc,
                                        torch::Tensor vc, torch::Tensor bt,
                                        torch::Tensor ctx, int64_t block_size,
                                        double scale, int64_t num_kv_heads,
                                        c10::optional<torch::Tensor> alibi,
                                        c10::optional<torch::Tensor> block_mask,
                                        int64_t window) {
  CK(q);
  CK(kc);
  CK(vc);
  ...
  dim3 grid(H, B);
#define LAUNCH(DD)                                                    \
  paged_attention<half, DD><<<grid, 32, 0, stream()>>>(               \
      hp(q), hp(kc), hp(vc), bt.data_ptr<int>(), ctx.data_ptr<int>(), \
      hpm(out), int(block_size), int(bt.size(1)), float(scale), H,    \
      int(num_kv_heads), al, al ? 1 : 0, bm, bm ? 1 : 0, int(window))
  if (D == 64)
    LAUNCH(64);
  else if (D == 128)
    LAUNCH(128);
  else
    TORCH_CHECK(false, "D must be 64/128");
#undef LAUNCH
  return out;
}
static torch::Tensor py_paged_attention_v2(
    torch::Tensor q, torch::Tensor kc, torch::Tensor vc, torch::Tensor bt,
    torch::Tensor ctx, int64_t block_size, double scale, int64_t num_kv_heads,
...
#define LAUNCH(DD)                                                             \
  do {                                                                         \
    paged_attention_partition<half, DD><<<pg, 32, 0, stream()>>>(              \
        hp(q), hp(kc), hp(vc), bt.data_ptr<int>(), ctx.data_ptr<int>(),        \
        tmp.data_ptr<float>(), ml.data_ptr<float>(), es.data_ptr<float>(),     \
        int(block_size), int(bt.size(1)), float(scale), H, int(num_kv_heads),  \
        P, int(partition_size), int(window));                                  \
    paged_attention_reduce<half, DD>                                           \
        <<<rg, 32, 0, stream()>>>(tmp.data_ptr<float>(), ml.data_ptr<float>(), \
                                  es.data_ptr<float>(), hpm(out), H, P);       \
  } while (0)
  if (D == 64)
    LAUNCH(64);
  else if (D == 128)
...
// ---- rope + insert ----
static void py_rope_kv_insert(torch::Tensor k, torch::Tensor v,
                              torch::Tensor cosb, torch::Tensor sinb,
                              torch::Tensor positions, torch::Tensor slots,
...
#define LAUNCH(DD, NORM)                                                 \
  rope_kv_insert<half, DD, NORM><<<M, 32, 0, stream()>>>(                \
      hp(k), hp(v), hp(cosb), hp(sinb), positions.data_ptr<int>(),       \
      slots.data_ptr<int64_t>(), hpm(kc), hpm(vc), w, int(num_kv_heads), \
      int(block_size), gemma ? 1 : 0, float(eps))
  if (D == 64) {
    if (w)
      LAUNCH(64, true);
    else
...
static torch::Tensor py_rope_q(torch::Tensor q, torch::Tensor cosb,
                               torch::Tensor sinb, torch::Tensor positions,
                               int64_t num_heads,
                               c10::optional<torch::Tensor> norm_weight,
                               bool gemma, double eps) {
  CK(q);
  ...
  const half* w = norm_weight ? hp(*norm_weight) : nullptr;
#define LAUNCH(DD, NORM)                                                 \
  rope_q<half, DD, NORM><<<M, 32, 0, stream()>>>(                        \
      hp(q), hp(cosb), hp(sinb), positions.data_ptr<int>(), hpm(out), w, \
      int(num_heads), gemma ? 1 : 0, float(eps))
  if (D == 64) {
    if (w)
      LAUNCH(64, true);
    else
...
static torch::Tensor py_attn_q(torch::Tensor q, torch::Tensor Kq,
                               torch::Tensor Vq, const std::string& fmt,
                               bool causal, double scale, int64_t B, int64_t H,
                               int64_t N, int64_t D) {
  CK(q);
  CK(Kq);
  CK(Vq);
...
#define LAUNCH(FMT, DD, C)                                                     \
  attn_q<tmq::FMT, DD, C><<<grid, 32, 0, stream()>>>(                          \
      hp(q), Kq.data_ptr<uint8_t>(), Vq.data_ptr<uint8_t>(), hpm(out), int(N), \
      int(H), float(scale))
#define DISPATCH(FMT)                         \
  do {                                        \
    if (D == 64) {                            \
...
// ---- MLA ----
static torch::Tensor py_mla_decode(torch::Tensor q, torch::Tensor kv_cache,
                                   torch::Tensor bt, torch::Tensor ctx,
  ...
  mla_decode<512, 64><<<dim3(H, B), 32, 0, stream()>>>(
      bp(q), bp(kv_cache), bt.data_ptr<int>(), ctx.data_ptr<int>(), bpm(out),
      int(block_size), int(bt.size(1)), float(scale), H);
  return out;
}
static torch::Tensor py_mla_decode_fp8(torch::Tensor q, torch::Tensor data,
                                       torch::Tensor scl, torch::Tensor bt,
                                       torch::Tensor ctx, int64_t block_size,
                                       double scale) {
  CK(q);
  CK(data);
  ...
  const int B = q.size(0), H = q.size(1);
  auto out = torch::empty_like(q);
  mla_decode_fp8<<<dim3(H, B), 32, 0, stream()>>>(
      bp(q), data.data_ptr<uint8_t>(), scl.data_ptr<uint8_t>(),
      bt.data_ptr<int>(), ctx.data_ptr<int>(), bpm(out), int(block_size),
      int(bt.size(1)), float(scale), H);
  return out;
}
static void py_mla_kv_insert_fp8(torch::Tensor kv, torch::Tensor cosb,
                                 torch::Tensor sinb, torch::Tensor positions,
                                 torch::Tensor slots, torch::Tensor data,
                                 torch::Tensor scl, int64_t block_size) {
  CK(kv);
  CK(cosb);
  CK(sinb);
  CK(positions);
  ...
  CK(data);
  CK(scl);
  mla_kv_insert_fp8<<<int(kv.size(0)), 32, 0, stream()>>>(
      bp(kv), bp(cosb), bp(sinb), positions.data_ptr<int>(),
      slots.data_ptr<int64_t>(), data.data_ptr<uint8_t>(),
      scl.data_ptr<uint8_t>(), int(block_size));
}
static torch::Tensor py_mla_q_norm_rope(
    torch::Tensor q, torch::Tensor cosb, torch::Tensor sinb,
    torch::Tensor positions, int64_t num_heads, int64_t nope_dim,
    int64_t rope_dim, int64_t norm_mode, double eps,
    c10::optional<torch::Tensor> norm_weight) {
  CK(q);
  CK(cosb);
  CK(sinb);
...
#define LAUNCH(DD)                                                    \
  mla_q_norm_rope<DD><<<M, 32, 0, stream()>>>(                        \
      bp(q), bp(cosb), bp(sinb), positions.data_ptr<int>(), bpm(out), \
      int(num_heads), int(nope_dim), int(rope_dim), int(norm_mode),   \
      float(eps), w)
  if (D == 128)
    LAUNCH(128);
  else if (D == 192)
    ...
    TORCH_CHECK(false, "D must be 128/192/256/512");
#undef LAUNCH
  return out;
}

// ---- samplers ----
static torch::Tensor py_sample(torch::Tensor logits, const std::string& mode,
                               int64_t seed, double temperature, double param,
                               int64_t K) {
  CK(logits);
  ...
  if (mode == "argmax")
    argmax_k<float><<<T, 32, 0, stream()>>>(lp, op, V);
  else if (mode == "categorical")
    sample_categorical<float>
        <<<T, 32, 0, stream()>>>(lp, op, V, unsigned(seed), invtemp);
  else if (mode == "top_k")
    top_k_sample<float>
        <<<T, 32, 0, stream()>>>(lp, op, V, int(K), unsigned(seed), invtemp);
  else if (mode == "top_p")
    top_p_sample<float><<<T, 32, 0, stream()>>>(lp, op, V, float(param),
                                                unsigned(seed), invtemp);
  else if (mode == "min_p")
    min_p_sample<float><<<T, 32, 0, stream()>>>(lp, op, V, float(param),
                                                unsigned(seed), invtemp);
  else if (mode == "typical_p")
    typical_p_sample<float><<<T, 32, 0, stream()>>>(lp, op, V, float(param),
                                                    unsigned(seed), invtemp);
  else
    TORCH_CHECK(false, "unknown sample mode ", mode);
  return out;
}
static torch::Tensor py_apply_penalties(
    torch::Tensor logits, torch::Tensor prev_tokens, torch::Tensor parent_ids,
    double temperature, double rep, double presence, double freq,
  ...
  penalty_histogram<<<(T * L + 255) / 256, 256, 0, stream()>>>(
      prev_tokens.data_ptr<int>(), counts.data_ptr<int>(), V, L, T * L,
      parent_ids.data_ptr<int>());
  apply_penalty<float><<<T, 32, 0, stream()>>>(
      logits.data_ptr<float>(), counts.data_ptr<int>(), out.data_ptr<float>(),
      V, temperature > 0 ? float(1.0 / temperature) : 1.0f, float(rep),
      float(presence), float(freq), bias.data_ptr<float>(), int(eos_id),
      int(min_length), int(gen_len));
  return out;
}
static torch::Tensor py_apply_token_bitmask(torch::Tensor logits,
                                            torch::Tensor bitmask) {
  CK(logits);
  CK(bitmask);
  const int T = logits.size(0), V = logits.size(1);
  auto out = torch::empty_like(logits);
  apply_token_bitmask<float><<<T, 32, 0, stream()>>>(
      logits.data_ptr<float>(),
      reinterpret_cast<const uint32_t*>(bitmask.data_ptr<int>()),
      out.data_ptr<float>(), V, int(bitmask.size(1)));
  return out;
}
static torch::Tensor py_apply_bad_words(torch::Tensor logits,
                                        torch::Tensor bad_ids,
                                        torch::Tensor bad_lens) {
  CK(logits);
  CK(bad_ids);
  CK(bad_lens);
  const int T = logits.size(0), V = logits.size(1);
  auto out = torch::empty_like(logits);
  apply_bad_words<float><<<T, 32, 0, stream()>>>(
      logits.data_ptr<float>(), out.data_ptr<float>(), bad_ids.data_ptr<int>(),
      bad_lens.data_ptr<int>(), V, int(bad_ids.size(1)));
  return out;
}
...
static std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> py_beam_advance(
  ...
  beam_topk_partials<float><<<rows, 32, 0, stream()>>>(
      logits.data_ptr<float>(), cum_log_probs.data_ptr<float>(),
      sc.data_ptr<float>(), tok.data_ptr<int>(), V, two_bm);
  beam_select<<<B, 32, 0, stream()>>>(sc.data_ptr<float>(), tok.data_ptr<int>(),
                                      nt.data_ptr<int>(), par.data_ptr<int>(),
                                      nc.data_ptr<float>(), int(BM), two_bm);
  return {nt, par, nc};
}
static std::tuple<torch::Tensor, torch::Tensor> py_spec_verify_linear(
  ...
  spec_verify_linear<<<B, 32, 0, stream()>>>(
      draft.data_ptr<int>(), draft_probs.data_ptr<float>(),
      target_probs.data_ptr<float>(), bonus.data_ptr<int>(),
      accept_u.data_ptr<float>(), out.data_ptr<int>(), cnt.data_ptr<int>(), S,
      V, unsigned(seed));
  return {out, cnt};
}
static std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>
py_build_dynamic_tree(torch::Tensor parents) {
  CK(parents);
  ...
  build_dynamic_tree<<<B, 32, 0, stream()>>>(
      parents.data_ptr<int>(), nt.data_ptr<int>(), ns.data_ptr<int>(),
      pos.data_ptr<int>(), N);
  return {nt, ns, pos};
}
static std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>
py_spec_verify_tree(torch::Tensor draft, torch::Tensor target_probs,
                    torch::Tensor nt, torch::Tensor ns,
  ...
  spec_verify_tree<<<B, 32, 0, stream()>>>(
      draft.data_ptr<int>(), target_probs.data_ptr<float>(), nt.data_ptr<int>(),
      ns.data_ptr<int>(), ai.data_ptr<int>(), at.data_ptr<int>(),
...
static std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> py_spec_compact(
    torch::Tensor out_tokens, torch::Tensor accepted_cnt,
  ...
  spec_compact<<<1, 256, 0, stream()>>>(
      out_tokens.data_ptr<int>(), accepted_cnt.data_ptr<int>(),
      seq_lens.data_ptr<int>(), pt.data_ptr<int>(), pp.data_ptr<int>(),
      cu.data_ptr<int>(), B, Sp1);
  return {pt, pp, cu};
}
static torch::Tensor py_spec_update_kv_meta(torch::Tensor seq_lens,
                                            torch::Tensor accepted_cnt) {
  CK(seq_lens);
  CK(accepted_cnt);
  const int B = seq_lens.numel();
  auto out = torch::empty_like(seq_lens);
  spec_update_kv_meta<<<(B + 255) / 256, 256, 0, stream()>>>(
      seq_lens.data_ptr<int>(), accepted_cnt.data_ptr<int>(),
      out.data_ptr<int>(), B);
  return out;
}
...
static void py_mla_kv_insert(torch::Tensor kv_c, torch::Tensor k_pe,
                             torch::Tensor cosb, torch::Tensor sinb,
                             torch::Tensor positions, torch::Tensor slots,
                             torch::Tensor kv_cache, int64_t block_size,
                             int64_t norm_mode, double eps,
                             c10::optional<torch::Tensor> norm_weight) {
  CK(kv_c);
  CK(k_pe);
  CK(cosb);
  ...
  const __nv_bfloat16* w = norm_weight ? bp(*norm_weight) : nullptr;
#define LAUNCH(L)                                                          \
  mla_kv_insert<L><<<T, 32, 0, stream()>>>(                                \
      bp(kv_c), bp(k_pe), bp(cosb), bp(sinb), positions.data_ptr<int>(),   \
      slots.data_ptr<int64_t>(), bpm(kv_cache), int(block_size), rope_dim, \
  ...
  if (LATENT == 128)
    ...
    TORCH_CHECK(false, "LATENT must be 128/256/512");
#undef LAUNCH
}
static torch::Tensor py_mla_decode_partition(
    torch::Tensor q, torch::Tensor kv_cache, torch::Tensor bt,
    torch::Tensor ctx, int64_t block_size, double scale, int64_t partition_size,
    int64_t max_context) {
  CK(q);
  CK(kv_cache);
  CK(bt);
  ...
  auto es = torch::empty({B, H, P}, opts);
  auto out = torch::empty({B, H, 512}, q.options());
  mla_decode_partition<512, 64><<<dim3(H, B, P), 32, 0, stream()>>>(
      bp(q), bp(kv_cache), bt.data_ptr<int>(), ctx.data_ptr<int>(),
      tmp.data_ptr<float>(), ml.data_ptr<float>(), es.data_ptr<float>(),
      int(block_size), int(bt.size(1)), float(scale), H, P,
      int(partition_size));
...
static torch::Tensor py_mla_decode_fp8_partition(
    torch::Tensor q, torch::Tensor data, torch::Tensor scl, torch::Tensor bt,
    torch::Tensor ctx, int64_t block_size, double scale, int64_t partition_size,
    int64_t max_context) {
  CK(q);
  CK(data);
  ...
  auto es = torch::empty({B, H, P}, opts);
  auto out = torch::empty_like(q);
  mla_decode_fp8_v<false, true><<<dim3(H, B, P), 32, 0, stream()>>>(
      bp(q), data.data_ptr<uint8_t>(), scl.data_ptr<uint8_t>(),
      bt.data_ptr<int>(), ctx.data_ptr<int>(), nullptr, nullptr, 0, nullptr,
      tmp.data_ptr<float>(), ml.data_ptr<float>(), es.data_ptr<float>(),
      int(block_size), int(bt.size(1)), float(scale), H, P,
      int(partition_size));
  paged_attention_reduce<__nv_bfloat16, 512><<<dim3(H, B), 32, 0, stream()>>>(
...
static torch::Tensor py_fp8_paged_mqa_logits(
  ...
  // 1M-token ceiling.
  const int local_max_len = int((max_model_len + token_shard_world_size - 1) /
                                token_shard_world_size);
  auto out =
      torch::empty({B, (long)local_max_len}, q.options().dtype(torch::kFloat));
  ...
  const int logical_nt = H > 32 ? 32 : short_context_threshold > 0 ? 8 : 16;
  const int max_tiles = int((local_max_len + logical_nt - 1) / logical_nt);
  const dim3 grid((unsigned)std::min(max_tiles, persistent_ctas), (unsigned)B);
  auto launch = [&]<int NT, int HEAD_WARPS, int TOKEN_GROUPS>() {
    const size_t smem =
        tms::indexer_paged_mqa_logits_smem<D, NT, HEAD_WARPS, TOKEN_GROUPS>();
    auto kern = tms::indexer_paged_mqa_logits<D, NT, HEAD_WARPS, TOKEN_GROUPS>;
    constexpr int threads = 32 * HEAD_WARPS * TOKEN_GROUPS > 128
    ...
    kern<<<grid, threads, smem, stream()>>>(
        ...
        kv_block_stride, (int)block_tables.size(1), local_max_len,
...
__global__ void indexer_pack_tp_candidates_k(const float* __restrict__ logits,
                                             const int* __restrict__ indices,
                                             float* __restrict__ packed,
                                             int rows, int k, int logit_stride,
                                             int token_shard_rank,
                                             int token_shard_world) {
  ...
  packed[2 * i + 1] =
      valid ? float(local_id * token_shard_world + token_shard_rank) : -1.0f;
}
...
__global__ void indexer_unpack_tp_scores_k(const float* __restrict__ packed,
                                           float* __restrict__ scores,
                                           int* __restrict__ lengths, int rows,
                                           int candidates) {
  const int i = int(blockIdx.x) * blockDim.x + threadIdx.x;
...
static std::tuple<torch::Tensor, torch::Tensor> py_indexer_unpack_tp_scores(
  ...
  auto lengths = torch::empty({rows}, packed.options().dtype(torch::kInt));
...
__global__ void indexer_resolve_tp_candidates_k(
  ...
  output[i] =
      pos >= 0 && pos < candidates
          ? __float2int_rn(packed[((size_t)row * candidates + pos) * 2 + 1])
          : -1;
}

static void py_indexer_resolve_tp_candidates(torch::Tensor packed,
                                             torch::Tensor positions,
                                             torch::Tensor output) {
  CKD(packed, torch::kFloat);
  CKD(positions, torch::kInt);
...
static torch::Tensor py_fp8_mqa_logits(torch::Tensor q, torch::Tensor k,
  ...
  kern<<<grid, 32 * NWARP, smem, stream()>>>(
      reinterpret_cast<const uint8_t*>(q.data_ptr()),
      reinterpret_cast<const uint8_t*>(k.data_ptr()), kscale.data_ptr<float>(),
...
// slot is 576 bf16 = 1152 B. Ampere (sm80) has no native fp8e4nv, so vLLM
// cannot store an fp8 KV cache there -- this is the geometry that actually runs
// on A100.
static torch::Tensor py_mla_decode_bf16_sparse_glm(
    torch::Tensor q, torch::Tensor kv, torch::Tensor bt, torch::Tensor indices,
    torch::Tensor topk_length, int64_t block_size, double scale) {
  CK(q);
  CK(kv);
  CK(bt);
  CK(indices);
  ...
  auto out = torch::empty({B, H, 512}, q.options());
  mla_decode_fp8_v<true, false, 576, 512, 0, 0>
      <<<dim3(H, B), 32, 0, stream()>>>(
          bp(q), reinterpret_cast<const uint8_t*>(kv.data_ptr()), nullptr,
          bt.data_ptr<int>(), nullptr, indices.data_ptr<int>(),
          topk_length.data_ptr<int>(), max_topk, bpm(out), nullptr, nullptr,
          nullptr, int(block_size), int(bt.size(1)), float(scale), H, 1, 0,
          1.0f);
  return out;
}
...
static torch::Tensor py_mla_decode_fp8_sparse_glm(
    torch::Tensor q, torch::Tensor data, torch::Tensor bt,
  ...
  auto out = torch::empty({B, H, 512}, q.options());
  if (partition_size <= 0) {
    mla_decode_fp8_v<true, false, 576, 512, 576, 1>
        <<<dim3(H, B), 32, 0, stream()>>>(
            bp(q), data.data_ptr<uint8_t>(), nullptr, bt.data_ptr<int>(),
            nullptr, indices.data_ptr<int>(), topk_length.data_ptr<int>(),
            max_topk, bpm(out), nullptr, nullptr, nullptr, int(block_size),
            int(bt.size(1)), float(scale), H, 1, 0, float(kv_scale));
    return out;
  }
  ...
  auto ml = torch::empty({B, H, P}, opts);
  auto es = torch::empty({B, H, P}, opts);
  mla_decode_fp8_v<true, true, 576, 512, 576, 1>
      <<<dim3(H, B, P), 32, 0, stream()>>>(
          bp(q), data.data_ptr<uint8_t>(), nullptr, bt.data_ptr<int>(), nullptr,
          indices.data_ptr<int>(), topk_length.data_ptr<int>(), max_topk,
          nullptr, tmp.data_ptr<float>(), ml.data_ptr<float>(),
          es.data_ptr<float>(), int(block_size), int(bt.size(1)), float(scale),
          H, P, int(partition_size), float(kv_scale));
  paged_attention_reduce<__nv_bfloat16, 512><<<dim3(H, B), 32, 0, stream()>>>(
      tmp.data_ptr<float>(), ml.data_ptr<float>(), es.data_ptr<float>(),
      bpm(out), H, P);
  return out;
...
static torch::Tensor py_mla_decode_fp8_sparse_glm_splitq(
    torch::Tensor q_nope, torch::Tensor q_pe, torch::Tensor data,
    torch::Tensor bt, torch::Tensor indices, torch::Tensor topk_length,
    int64_t block_size, double scale, double kv_scale, int64_t partition_size) {
  CK(q_nope);
  CK(data);
  CK(bt);
  CK(indices);
  ...
  if (partition_size <= 0) {
    mla_decode_fp8_v<true, false, 576, 512, 576, 1>
        <<<dim3(H, B), 32, 0, stream()>>>(
            bp(q_nope), data.data_ptr<uint8_t>(), nullptr, bt.data_ptr<int>(),
            nullptr, indices.data_ptr<int>(), topk_length.data_ptr<int>(),
            max_topk, bpm(out), nullptr, nullptr, nullptr, int(block_size),
            int(bt.size(1)), float(scale), H, 1, 0, float(kv_scale), bp(q_pe),
            pe_stride);
    return out;
  }
  const int P = int((max_topk + partition_size - 1) / partition_size);
  ...
  mla_decode_fp8_v<true, true, 576, 512, 576, 1>
      <<<dim3(H, B, P), 32, 0, stream()>>>(
          bp(q_nope), data.data_ptr<uint8_t>(), nullptr, bt.data_ptr<int>(),
          nullptr, indices.data_ptr<int>(), topk_length.data_ptr<int>(),
          max_topk, nullptr, tmp.data_ptr<float>(), ml.data_ptr<float>(),
          es.data_ptr<float>(), int(block_size), int(bt.size(1)), float(scale),
          H, P, int(partition_size), float(kv_scale), bp(q_pe), pe_stride);
  paged_attention_reduce<__nv_bfloat16, 512><<<dim3(H, B), 32, 0, stream()>>>(
      tmp.data_ptr<float>(), ml.data_ptr<float>(), es.data_ptr<float>(),
      bpm(out), H, P);
  return out;
...
static void py_moe_weighted_sum(torch::Tensor x, torch::Tensor w,
                                torch::Tensor out) {
  CK(x);
  CK(w);
  CK(out);
  TORCH_CHECK(x.scalar_type() == torch::kBFloat16 &&
                  ...
                  w.scalar_type() == torch::kFloat,
              "moe_weighted_sum: bf16 x/out, f32 w");
  const int T = x.size(0), K = x.size(1), Hdim = x.size(2);
  TORCH_CHECK(w.size(0) == T && w.size(1) == K && out.size(0) == T &&
                  out.size(1) == Hdim,
...
static torch::Tensor py_sparse_topk_tlen(torch::Tensor indices) {
  ...
  if (rows > 0)
    sparse_topk_tlen<<<rows, 256, 0, stream()>>>(indices.data_ptr<int>(),
                                                 tlen.data_ptr<int>(), topk);
  return tlen;
}
...
static int dsv4_partitions(int max_topk, int partition_size) {
  ...
  return std::min(int((max_topk + partition_size - 1) / partition_size),
...
static void launch_mla_decode_fp8_sparse_dsv4_merged(
    torch::Tensor q, torch::Tensor main_cache, torch::Tensor main_bt,
    ...
    bool extra_indices_are_slots, int extra_block_size, int extra_partitions,
    double scale, int partition_size, float* tmp, float* ml, float* es) {
  const int B = q.size(0), H = q.size(1);
  const int main_launched =
      std::min(main_partitions, dsv4_mla_persistent_partitions());
  const int extra_launched =
      std::min(extra_partitions, dsv4_mla_persistent_partitions());
...
#define QC_DSV4_MERGED_ARGS                                                    \
  bp(q), main_data, main_data,                                                 \
      ...
      SCALE_SLOT_STRIDE_BYTES, total_partitions, 0, main_indices_are_slots,    \
      true, extra_data, extra_data,                                            \
      ...
      extra_bt.numel() > 0 ? int(extra_bt.size(1)) : 0, extra_partitions,      \
      partition_size, 1.0f, extra_page_stride_bytes,                           \
      extra_scale_block_offset_bytes, SCALE_SLOT_STRIDE_BYTES,                 \
      main_partitions, extra_indices_are_slots, main_launched, extra_launched, \
      int(main_cache.size(0)), int(extra_cache.size(0))
  if (bf16_pages) {
...
static torch::Tensor py_mla_decode_fp8_sparse_dsv4(
    torch::Tensor q, torch::Tensor main_cache, torch::Tensor main_bt,
    torch::Tensor main_indices, torch::Tensor main_topk_length,
    bool main_indices_are_slots, torch::Tensor extra_cache,
    torch::Tensor extra_bt, torch::Tensor extra_indices,
    torch::Tensor extra_topk_length, bool extra_indices_are_slots,
    c10::optional<torch::Tensor> sink, int64_t main_block_size,
    int64_t extra_block_size, double scale, int64_t partition_size) {
  CK(q);
  CK(main_indices);
  CK(main_topk_length);
  CK(extra_indices);
  CK(extra_topk_length);
  ...
  TORCH_CHECK(main_indices.scalar_type() == torch::kInt,
              "main_indices must be int32");
  TORCH_CHECK(extra_indices.scalar_type() == torch::kInt,
              "extra_indices must be int32");
  TORCH_CHECK(q.size(2) == 512, "DSV4 MLA expects q width 512, got ",
              q.size(2));
  const int B = q.size(0), H = q.size(1);
  ...
  if (sink)
    TORCH_CHECK(sink->size(0) >= H, "sink must have at least H entries");

  int ps = int(partition_size);
  if (dsv4_mla_adaptive_partitions_enabled() && extra_indices.size(1) >= 1024 &&
      ps > 2)
    ps = 2;
  ...
  if (ps <= 0) {
    const int max_width =
        std::max(int(main_indices.size(1)), int(extra_indices.size(1)));
  ...
  auto tmp = tmp_sentinel ? torch::full({B, H, total_p, 512}, 12345.0f, opts)
  ...
  launch_mla_decode_fp8_sparse_dsv4_merged(
      ...
      main_indices_are_slots, int(main_block_size), main_p, extra_cache,
      extra_bt, extra_indices, extra_topk_length, extra_indices_are_slots,
      int(extra_block_size), extra_p, scale, ps, tmp.data_ptr<float>(),
      ml.data_ptr<float>(), es.data_ptr<float>());
  ...
  if (debug_partials) {
    ...
    if (tmp_sentinel) {
      auto written_parts =
          torch::isfinite(ml) & (ml > -std::numeric_limits<float>::infinity());
    ...
    if (ml_bad || es_bad || tmp_bad || sentinel_left) {
      auto ml_rows = (torch::isnan(ml) | torch::isinf(ml))
                         ...
                         .flatten();
      auto per_part =
          (torch::isnan(tmp) | torch::isinf(tmp)).any(-1).sum({0, 1});
      printf(
          "MLA_DEBUG_PARTIALS: B=%d H=%d main_p=%d extra_p=%d "
          "ml_bad=%ld es_bad=%ld tmp_bad=%ld sentinel_in_written=%ld "
          "first_bad_batch=%ld\n",
          B, H, main_p, extra_p, long(ml_bad), long(es_bad), long(tmp_bad),
          long(sentinel_left),
  ...
  dsv4_attention_reduce_active_channels<__nv_bfloat16, 512>
      <<<dim3(H, B), 512, total_p * sizeof(float), stream()>>>(
          ...
          sink ? sink->data_ptr<float>() : nullptr, bpm(out), H, main_p,
          extra_p, total_p);
  return out;
}

static torch::Tensor py_mla_decode_fp8_sparse(
    torch::Tensor q, torch::Tensor data, torch::Tensor scl, torch::Tensor bt,
    torch::Tensor indices, torch::Tensor topk_length, int64_t block_size,
  ...
  auto out = torch::empty_like(q);
  if (partition_size <= 0) {
    mla_decode_fp8_v<true, false><<<dim3(H, B), 32, 0, stream()>>>(
        bp(q), data.data_ptr<uint8_t>(), scl.data_ptr<uint8_t>(),
        bt.data_ptr<int>(), nullptr, indices.data_ptr<int>(),
        topk_length.data_ptr<int>(), max_topk, bpm(out), nullptr, nullptr,
        nullptr, int(block_size), int(bt.size(1)), float(scale), H, 1, 0);
  ...
  auto ml = torch::empty({B, H, P}, opts);
  auto es = torch::empty({B, H, P}, opts);
  mla_decode_fp8_v<true, true><<<dim3(H, B, P), 32, 0, stream()>>>(
      bp(q), data.data_ptr<uint8_t>(), scl.data_ptr<uint8_t>(),
      bt.data_ptr<int>(), nullptr, indices.data_ptr<int>(),
      topk_length.data_ptr<int>(), max_topk, nullptr, tmp.data_ptr<float>(),
      ml.data_ptr<float>(), es.data_ptr<float>(), int(block_size),
      int(bt.size(1)), float(scale), H, P, int(partition_size));
  paged_attention_reduce<__nv_bfloat16, 512><<<dim3(H, B), 32, 0, stream()>>>(
      tmp.data_ptr<float>(), ml.data_ptr<float>(), es.data_ptr<float>(),
      bpm(out), H, P);
  return out;
}
static torch::Tensor py_paged_attention_gqa_staged(
    torch::Tensor q, torch::Tensor kc, torch::Tensor vc, torch::Tensor bt,
    torch::Tensor ctx, int64_t block_size, double scale, int64_t num_kv_heads) {
  CK(q);
  CK(kc);
...
#define LAUNCH(DD)                                                      \
  paged_attention_gqa_staged<half, DD><<<grid, 32 * gs, 0, stream()>>>( \
      hp(q), hp(kc), hp(vc), bt.data_ptr<int>(), ctx.data_ptr<int>(),   \
      hpm(out), int(block_size), int(bt.size(1)), float(scale), H,      \
      int(num_kv_heads))
  if (D == 64)
...
static torch::Tensor py_attn_window(torch::Tensor q, torch::Tensor k,
...
#define LAUNCH(DD)                                  \
  attn_window<half, DD><<<grid, 32, 0, stream()>>>( \
...
static void py_turboquant_store_fp8(torch::Tensor key, torch::Tensor value,
                                    ...
                                    int64_t num_kv_heads, int64_t kps,
                                    int64_t vqb) {
  CK(key);
  CK(value);
  CK(slot_mapping);
  CKTQ(kv_cache);
  TORCH_CHECK(
      kv_cache.scalar_type() == torch::kUInt8 && kv_cache.stride(3) == 1,
  ...
  if (NH == 0) return;
#define LAUNCH(T, RD, ST)                                                  \
  tq_store_fp8<T, ST><<<NH, 32, 0, stream()>>>(                            \
...
#define DISPATCH_SLOT(T, RD)                        \
  do {                                              \
    if (slot_mapping.scalar_type() == torch::kLong) \
      LAUNCH(T, RD, int64_t);                       \
    else                                            \
      LAUNCH(T, RD, int);                           \
  } while (0)
  if (key.scalar_type() == torch::kHalf)
    DISPATCH_SLOT(half, hp);
  else if (key.scalar_type() == torch::kBFloat16)
    DISPATCH_SLOT(__nv_bfloat16, bp);
  else
    TORCH_CHECK(false, "turboquant_store_fp8: key must be fp16/bf16");
#undef DISPATCH_SLOT
#undef LAUNCH
}

static void py_turboquant_store_mse(
    torch::Tensor y, torch::Tensor norms, torch::Tensor value,
    torch::Tensor midpoints, torch::Tensor kv_cache, torch::Tensor slot_mapping,
    int64_t num_kv_heads, int64_t mse_bits, int64_t kps, int64_t vqb) {
  CK(y);
  CK(norms);
  CK(value);
  CK(midpoints);
  ...
  TORCH_CHECK(
      ...
      "kv_cache must be uint8 with contiguous slots");
  TORCH_CHECK(
      y.scalar_type() == torch::kFloat && value.scalar_type() == torch::kFloat,
  ...
  if (NH == 0) return;
#define LAUNCH(ST)                                                           \
  tq_store_mse<ST><<<NH, 32, 0, stream()>>>(                                 \
...
static void py_turboquant_decode_stage1(
    ...
    int64_t num_splits, int64_t mse_bits, int64_t kps, int64_t vqb,
    double scale, bool key_fp8, bool norm_correction, int64_t window) {
  CK(centroids);
  CKTQ(q_rot);
  CKTQ(kv_cache);
  CKTQ(block_table);
  ...
  CKTQ(mid_o);
  TORCH_CHECK(
      kv_cache.stride(3) == 1 && q_rot.stride(2) == 1 && mid_o.stride(3) == 1,
  ...
  TORCH_CHECK(mid_o.dim() == 4 && mid_o.size(0) >= B && mid_o.size(1) >= Hq &&
                  mid_o.size(2) >= num_splits && mid_o.size(3) >= D + 1,
...
#define LAUNCH(T, RD)                                                          \
  tq_decode_stage1<T><<<grid, 32, 0, stream()>>>(                              \
      ...
      block_table.stride(0), mid_o.stride(0), mid_o.stride(1),                 \
      mid_o.stride(2), D, block_size, int(num_splits), Hq / Hk, int(mse_bits), \
      mse_bytes, int(kps), int(vqb), vdb, float(scale), key_fp8 ? 1 : 0,       \
      norm_correction ? 1 : 0, int(window))
  if (q_rot.scalar_type() == torch::kFloat)
    LAUNCH(float, fp);
  else if (q_rot.scalar_type() == torch::kHalf)
    LAUNCH(half, hp);
  else if (q_rot.scalar_type() == torch::kBFloat16)
    LAUNCH(__nv_bfloat16, bp);
  else
    TORCH_CHECK(false, "turboquant_decode_stage1: q must be fp32/fp16/bf16");
#undef LAUNCH
}

static void py_turboquant_decode_stage2(torch::Tensor mid_o,
  ...
  dim3 grid(B, Hq);
#define LAUNCH(T, WP)                                                     \
  tq_decode_stage2<T><<<grid, 32, 0, stream()>>>(                         \
  ...
  if (output.scalar_type() == torch::kHalf)
    ...
    LAUNCH(__nv_bfloat16, bpm);
  else if (output.scalar_type() == torch::kFloat)
    LAUNCH(float, fpm);
  else
    TORCH_CHECK(false,
                "turboquant_decode_stage2: output must be fp32/fp16/bf16");
#undef LAUNCH
}

static void py_turboquant_dequant_kv(torch::Tensor kv_cache,
                                     torch::Tensor block_table,
                                     torch::Tensor centroids,
                                     torch::Tensor k_out, torch::Tensor v_out,
                                     int64_t num_positions, int64_t mse_bits,
                                     int64_t kps, int64_t vqb, bool key_fp8,
  ...
  CKTQ(v_out);
  TORCH_CHECK(
      kv_cache.stride(3) == 1 && k_out.stride(3) == 1 && v_out.stride(3) == 1,
      "innermost dims must be contiguous");
  ...
  tq_full_dequant_kv<<<grid, 32, 0, stream()>>>(
      ...
      centroids.data_ptr<float>(), hpm(k_out), hpm(v_out), k_out.stride(0),
      k_out.stride(1), k_out.stride(2), v_out.stride(0), v_out.stride(1),
      v_out.stride(2), kv_cache.stride(0), kv_cache.stride(1),
      kv_cache.stride(2), block_table.stride(0), D, block_size, Hk, mse_bytes,
      int(kps), int(vqb), vdb, int(mse_bits), key_fp8 ? 1 : 0,
      norm_correction ? 1 : 0);
}
...
void init_serving(py::module_& m) {
  ...
  m.def("fill_short_context_topk_indices", &py_fill_short_context_topk_indices,
        py::arg("output"), py::arg("positions"), py::arg("topk"),
        py::arg("compress_ratio"));
  m.def("dsv4_mhc_pre", &py_dsv4_mhc_pre, py::arg("residual"), py::arg("fn"),
        py::arg("hc_scale"), py::arg("hc_base"), py::arg("rms_eps"),
        py::arg("pre_eps"), py::arg("sinkhorn_eps"), py::arg("post_multiplier"),
        py::arg("sinkhorn_repeat"), py::arg("norm_weight") = c10::nullopt,
        py::arg("norm_eps") = 0.0);
  m.def("dsv4_mhc_fused_post_pre", &py_dsv4_mhc_fused_post_pre, py::arg("x"),
        py::arg("residual"), py::arg("post_mix"), py::arg("comb_mix"),
        py::arg("fn"), py::arg("hc_scale"), py::arg("hc_base"),
        py::arg("rms_eps"), py::arg("pre_eps"), py::arg("sinkhorn_eps"),
        py::arg("post_multiplier"), py::arg("sinkhorn_repeat"),
        py::arg("norm_weight") = c10::nullopt, py::arg("norm_eps") = 0.0);
  m.def("dsv4_mhc_post", &py_dsv4_mhc_post, py::arg("x"), py::arg("residual"),
        py::arg("post_mix"), py::arg("comb_mix"));
  m.def("dsv4_hc_head", &py_dsv4_hc_head, py::arg("residual"), py::arg("fn"),
        py::arg("hc_scale"), py::arg("hc_base"), py::arg("rms_eps"),
        py::arg("hc_eps"));
  m.def("mla_kv_insert", &py_mla_kv_insert, py::arg("kv_c"), py::arg("k_pe"),
        py::arg("cos"), py::arg("sin"), py::arg("positions"),
        py::arg("slot_mapping"), py::arg("kv_cache"), py::arg("block_size"),
        py::arg("norm_mode") = 0, py::arg("eps") = 1e-6,
  ...
  m.def("fp8_paged_mqa_logits", &py_fp8_paged_mqa_logits, py::arg("q"),
        py::arg("kv_cache"), py::arg("weights"), py::arg("context_lens"),
        py::arg("block_tables"), py::arg("max_model_len"),
        py::arg("token_shard_rank") = 0, py::arg("token_shard_world_size") = 1,
  ...
  m.def("fp8_mqa_logits", &py_fp8_mqa_logits, py::arg("q"), py::arg("k"),
        py::arg("kscale"), py::arg("weights"), py::arg("ks"), py::arg("ke"));
  ...
  m.def("mla_decode_bf16_sparse_glm", &py_mla_decode_bf16_sparse_glm,
        py::arg("q"), py::arg("kv"), py::arg("block_table"), py::arg("indices"),
        py::arg("topk_length"), py::arg("block_size"), py::arg("scale"));
  m.def("mla_decode_fp8_sparse_glm", &py_mla_decode_fp8_sparse_glm,
        py::arg("q"), py::arg("data"), py::arg("block_table"),
  ...
  m.def("mla_decode_fp8_sparse_glm_splitq",
        &py_mla_decode_fp8_sparse_glm_splitq, py::arg("q_nope"),
        py::arg("q_pe"), py::arg("data"), py::arg("block_table"),
        py::arg("indices"), py::arg("topk_length"), py::arg("block_size"),
        py::arg("scale"), py::arg("kv_scale"), py::arg("partition_size") = 0);
  m.def("moe_weighted_sum", &py_moe_weighted_sum, py::arg("x"), py::arg("w"),
        py::arg("out"));
  m.def("mla_decode_fp8_sparse", &py_mla_decode_fp8_sparse, py::arg("q"),
        py::arg("data"), py::arg("scale_cache"), py::arg("block_table"),
        py::arg("indices"), py::arg("topk_length"), py::arg("block_size"),
        py::arg("scale"), py::arg("partition_size") = 0);
  m.def("mla_decode_fp8_sparse_dsv4", &py_mla_decode_fp8_sparse_dsv4,
        py::arg("q"), py::arg("main_cache"), py::arg("main_block_table"),
        py::arg("main_indices"), py::arg("main_topk_length"),
        py::arg("main_indices_are_slots"), py::arg("extra_cache"),
        py::arg("extra_block_table"), py::arg("extra_indices"),
        py::arg("extra_topk_length"), py::arg("extra_indices_are_slots"),
        py::arg("sink"), py::arg("main_block_size"),
        py::arg("extra_block_size"), py::arg("scale"),
        py::arg("partition_size") = 0);
  m.def("paged_attention_gqa_staged", &py_paged_attention_gqa_staged);
  m.def("attn_window", &py_attn_window, py::arg("q"), py::arg("k"),
        py::arg("v"), py::arg("scale"), py::arg("window") = 0);
  m.def("kv_scatter", &py_kv_scatter);
  m.def("kv_gather", &py_kv_gather);
  m.def("copy_blocks", &py_copy_blocks);
  m.def("paged_attention", &py_paged_attention, py::arg("q"),
        py::arg("key_cache"), py::arg("value_cache"), py::arg("block_table"),
        py::arg("context_lens"), py::arg("block_size"), py::arg("scale"),
        py::arg("num_kv_heads"), py::arg("alibi") = py::none(),
        py::arg("block_mask") = py::none(), py::arg("window") = 0);
  m.def("paged_attention_v2", &py_paged_attention_v2, py::arg("q"),
        py::arg("key_cache"), py::arg("value_cache"), py::arg("block_table"),
        py::arg("context_lens"), py::arg("block_size"), py::arg("scale"),
        py::arg("num_kv_heads"), py::arg("partition_size"),
        py::arg("max_context"), py::arg("window") = 0);
  m.def("rope_kv_insert", &py_rope_kv_insert, py::arg("k"), py::arg("v"),
        py::arg("cos"), py::arg("sin"), py::arg("positions"),
        py::arg("slot_mapping"), py::arg("key_cache"), py::arg("value_cache"),
        py::arg("num_kv_heads"), py::arg("block_size"),
        py::arg("norm_weight") = py::none(), py::arg("gemma") = false,
        py::arg("eps") = 1e-6);
  m.def("rope_q", &py_rope_q, py::arg("q"), py::arg("cos"), py::arg("sin"),
        py::arg("positions"), py::arg("num_heads"),
        py::arg("norm_weight") = py::none(), py::arg("gemma") = false,
        py::arg("eps") = 1e-6);
  ...
  m.def("mla_q_norm_rope", &py_mla_q_norm_rope, py::arg("q"), py::arg("cos"),
        py::arg("sin"), py::arg("positions"), py::arg("num_heads"),
        py::arg("nope_dim"), py::arg("rope_dim"), py::arg("norm_mode") = 0,
        py::arg("eps") = 1e-6, py::arg("norm_weight") = py::none());
  m.def("sample", &py_sample, py::arg("logits"), py::arg("mode"),
        py::arg("seed") = 0, py::arg("temperature") = 1.0,
        py::arg("param") = 0.9, py::arg("k") = 40);
  ...
  m.def("turboquant_store_fp8", &py_turboquant_store_fp8, py::arg("key"),
        py::arg("value"), py::arg("kv_cache"), py::arg("slot_mapping"),
        py::arg("num_kv_heads"), py::arg("key_packed_size"),
        py::arg("value_quant_bits"));
  m.def("turboquant_store_mse", &py_turboquant_store_mse, py::arg("y"),
        py::arg("norms"), py::arg("value"), py::arg("midpoints"),
  ...
  m.def("turboquant_dequant_kv", &py_turboquant_dequant_kv, py::arg("kv_cache"),
        py::arg("block_table"), py::arg("centroids"), py::arg("k_out"),
        py::arg("v_out"), py::arg("num_positions"), py::arg("mse_bits"),
        py::arg("key_packed_size"), py::arg("value_quant_bits"),
        py::arg("key_fp8"), py::arg("norm_correction"));
}
