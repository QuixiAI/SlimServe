 ...
 * the V2 GPU worker
 * (vllm/v1/worker/gpu/{buffer_utils,input_batch,block_table}.py,
...
template <typename T>
__global__ void apply_write(T* __restrict__ out, long row_stride,
...
__global__ void apply_write_multi(
    ...
    const long* __restrict__ out_strides, const int* __restrict__ write_indices,
    const int* __restrict__ write_starts,
  ...
  int* row_ptr =
      reinterpret_cast<int*>(out_ptrs[g]) + (long)row * out_strides[g] + start;
  for (int i = threadIdx.x; i < len; i += blockDim.x)
    row_ptr[i] = write_contents[cu_start + i];
}
...
__global__ void prepare_prefill_inputs(
  ...
  const int* req_ptr =
      all_token_ids + (long)req_state_idx * all_token_ids_stride;
  for (int i = threadIdx.x; i < query_len; i += blockDim.x)
...
__global__ void combine_sampled_and_draft_tokens(
    ...
    const int* __restrict__ prefill_len, const long* __restrict__ draft_tokens,
    long draft_tokens_stride, const int* __restrict__ cu_num_logits,
    long* __restrict__ logits_indices, int num_new_sampled_tokens) {
  const int b = blockIdx.x;
  const int req_state_idx = idx_mapping[b];
...
__global__ void post_update(
    ...
    long* __restrict__ last_sampled_tokens, int* __restrict__ output_bin_counts,
    long output_bin_counts_stride, const long* __restrict__ sampled_tokens,
    long sampled_tokens_stride, const int* __restrict__ num_sampled,
    const int* __restrict__ num_rejected,
    const int* __restrict__ query_start_loc, int* __restrict__ all_token_ids,
    long all_token_ids_stride, int* __restrict__ total_len, int num_reqs,
    int sampled_cols, long vocab_size, long* __restrict__ diag) {
  ...
  const int ns = num_sampled[r];
  {
    ...
    if (bad != 0) {
      if (diag != nullptr &&
          atomicAdd(reinterpret_cast<unsigned long long*>(&diag[0]), 1ull) ==
              0ull) {
        diag[1] = bad;
        diag[2] = r;
...
__global__ void expand_idx_mapping(const int* __restrict__ idx_mapping,
                                   int* __restrict__ expanded_idx_mapping,
...
__global__ void compute_slot_mappings(
  ...
  for (int i = start + threadIdx.x; i < end; i += blockDim.x) {
    ...
    if (cp_size == 1) {
      ...
      const bool is_local = (block_off / cp_interleave) % cp_size == cp_rank;
      const long rounds = block_off / ((long)cp_interleave * cp_size);
      const long local_off = rounds * cp_interleave + block_off % cp_interleave;
  ...
  for (int j = threadIdx.x; j < max_tokens_per_req; j += blockDim.x) {
    if (j < num_ctx) {
      ...
      if (query_off >= sample_off) {
        const long si =
            (long)req_idx * num_speculative_steps + (query_off - sample_off);
        out_sample_indices[si] = query_idx;
        out_sample_pos[si] = sample_from_anchor ? query_pos + 1 : query_pos;
