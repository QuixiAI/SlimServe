...
torch::Tensor marlin_int4_fp8_preprocess(
  ...
  if (!qzeros_or_none.has_value()) {
    ...
    hipLaunchKernelGGL(marlin_int4_fp8_preprocess_kernel_without_zp,
    ...
    hipLaunchKernelGGL(marlin_int4_fp8_preprocess_kernel_awq,
                       blocks, dim3(32), 0, stream,
                       (const int32_t*)qweight.data_ptr(),
                       (int32_t*)output.data_ptr(),
                       (const int32_t*)qzeros.data_ptr(), size_n, size_k,
                       group_size);
  }
